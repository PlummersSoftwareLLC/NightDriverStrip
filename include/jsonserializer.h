...
//
// History:     Mar-29-2023         Rbergen      Created for
// NightDriverStrip
//
//---------------------------------------------------------------------------
...
template <class E> constexpr auto to_value(E e) noexcept
{
...
inline bool SetIfNotOverflowed(JsonDocument &jsonDoc,
...
{
template <> struct Converter<CRGB>
{
...
template <> struct Converter<CRGBPalette16>
{
    ...
    static bool checkJson(JsonVariantConst src)
    {
        return src.is<JsonArrayConst>() &&
               src.as<JsonArrayConst>().size() == 16;
    }
};

inline bool canConvertFromJson(JsonVariantConst src,
                               const CRGBPalette16 &)
{
...
class JSONWriter
{
    // We allow the main JSON Writer task entry point function to access
    // private members
...
private:
    ...
    struct WriterEntry
    {
        ...
        WriterEntry(WriterEntry &&entry) noexcept :
            WriterEntry(entry.writer)
        {
        }
    };
...
public:
    // Add a writer to the collection. Returns the index of the added
    // writer, for use with FlagWriter()
