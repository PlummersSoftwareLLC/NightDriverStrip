...
#define ADD_CHANNEL(channel)                                             \
    debugI("Adding %zu LEDs to pin %d from channel %d on FastLED.",      \
           devices[channel]->GetLEDCount(), LED_PIN##channel, channel);  \
    FastLED                                                              \
        .addLeds<WS2812, LED_PIN##channel, COLOR_ORDER>(                 \
            devices[channel]->leds, devices[channel]->GetLEDCount())     \
        .setRgbw(Rgbw(kRGBWDefaultColorTemp,                             \
...
#define ADD_CHANNEL(channel)                                             \
    debugI("Adding %zu LEDs to pin %d from channel %d on FastLED.",      \
           devices[channel]->GetLEDCount(), LED_PIN##channel, channel);  \
    FastLED.addLeds<WS2812B, LED_PIN##channel, COLOR_ORDER>(             \
        devices[channel]->leds, devices[channel]->GetLEDCount());        \
    pinMode(LED_PIN##channel, OUTPUT)
#endif
        ...
        // The following "unrolled conditional compile loop" to set up the
        // channels is needed because the LED pin
    ...
    LEDStripGFX(size_t w, size_t h) : GFXBase(w, h)
    {
        ...
        if (!leds)
            throw std::runtime_error(
                "Unable to allocate LEDs in LEDStripGFX");
    }
...
#error The maximum value of NUM_CHANNELS (number of parallel channels) is 8
        ...
        for (int i = 0; i < NUM_CHANNELS; i++)
        {
            ...
            devices.push_back(make_shared_psram<LEDStripGFX>(
    ...
    // PostProcessFrame
    //
    // PostProcessFrame sends the data to the LED strip.  If it's fewer
    // than the size of the strip, we only send that many.
...
// HexagonGFX
//
// A version of the LEDStripGFX class that accounts for the layout of the
// hexagon LEDs
//
...
#error The maximum value of NUM_CHANNELS (number of parallel channels) is 8
    ...
    virtual int getStartIndexOfRow(int row) const
    {
        ...
        {
            // For the top half, we use the formula for the sum of an
            // arithmetic series.  It's ok to stare.
            return (row * (row + HEX_MAX_DIMENSION)) / 2;
        }
        else if (row < HEX_MAX_DIMENSION)
        {
            // For the bottom half, we adjust our row number to start from
            // 1 again (d = row - 9) Then work our way back up from the
            // end.  ChatGPT couldn't solve the series so I decided to
            // just work backwards from the end using the same formula as
            // the top half, and it works.
        ...
        else
        {
            ...
            throw std::runtime_error(
                str_sprintf(
                    "Tried to get index of row %d in the hexagon.", row)
                    .c_str());
        }
    }

    // Function to calculate the width of a specified row on an LED
    // display.
    virtual int getRowWidth(int row) const
    {
        ...
        {
            // For the bottom half, the width decreases from 19 back
            // to 10.
    ...
    // xy
    //
    // Absent a great way to map x/y coords to the hexagon face, we do it
    // by returning the Xth pixel in row Y.  It's up to you not to overrun
    // the width of that row, but it will just blend into the next row if
    // you do.

    __attribute__((always_inline)) virtual uint16_t xy(
        uint16_t x, uint16_t y) const noexcept override
    {
    ...
    // filHexRing
    //
    // Fills a ring around the hexagon, inset by the indent specified and
    ...
    virtual void fillHexRing(uint16_t indent, CRGB color)
    {
        ...
        for (int row = indent; row < HEX_MAX_DIMENSION - indent; ++row)
        {
            ...
            setPixel(startIndex + getRowWidth(row) - 1 - indent,
                     color);                      // last pixel
        }

        for (int i = indent;
             i < getRowWidth(HEX_MAX_DIMENSION - indent - 1) - indent;
             ++i)
            setPixel(getStartIndexOfRow(HEX_MAX_DIMENSION - indent - 1) +
                         i,
                     color);
    }
};
