...
inline String get_mac_address()
{
    uint8_t mac[6];
    WiFi.macAddress(mac);
    return str_sprintf("%02x%02x%02x%02x%02x%02x", mac[0], mac[1], mac[2],
                       mac[3], mac[4], mac[5]);
}
...
inline String get_mac_address_pretty()
{
    uint8_t mac[6];
    WiFi.macAddress(mac);
    return str_sprintf("%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1],
                       mac[2], mac[3], mac[4], mac[5]);
}

// NetworkReader
//
// Allows functions to be registered that are called at regular intervals
// and/or on request, in the background. As the name of the class implies,
// this is intended to be used to execute network requests, like for
// effects that require data from RESTful APIs.

class NetworkReader
{
    // We allow the main network task entry point function to access
    // private members
...
private:
    ...
    struct ReaderEntry
    {
        ...
        ReaderEntry(std::function<void()> reader,
        ...
        ReaderEntry(std::function<void()> reader, unsigned long interval,
                    unsigned long lastReadMs, bool flag, bool canceled) :
            reader(std::move(reader)),
            readInterval(interval),
        ...
        ReaderEntry(ReaderEntry &&entry) noexcept :
            ReaderEntry(entry.reader, entry.readInterval,
                        entry.lastReadMs, entry.flag, entry.canceled)
        {
        }
    };
...
public:
    // Add a reader to the collection. Returns the index of the added
    // reader, for use with FlagReader().
    //   Note that if an interval (in ms) is specified, the reader will
    //   run for the first time after the interval has passed, unless
    //   "true" is passed to the last parameter.
    size_t RegisterReader(const std::function<void()> &reader,
                          unsigned long interval = 0, bool flag = false);
