...
//              Apr-20-2019         Davepl      Adapted from Spectrum
//              Analyzer Aug-09-2025         Davepl      Substantively
//              rewritten to use C++20 features
//
//---------------------------------------------------------------------------
...
// Defaults below start with Mesmerizer values; others can diverge over
// time.
...
struct AudioInputParams
{
    ...
    float envFloorFromNoise; // Multiplier on mean noise floor to cap
                             // normalization
    float frameSNRGate;  // Gate frame if raw SNR (max/noiseMax) is below
                         // this
    float postScale;     // Post-scale for normalized band values (applied
                         // before clamp)
    float compressGamma; // Exponent for compression on normalized v
                         // (e.g., 1/3=cuberoot, 1/6=sqrt(cuberoot))
    float quietEnvFloorGate; // Gate entire frame if envFloor
                             // (noiseMean*envFloorFromNoise) < this (0
                             // disables)
    float liveAttackPerSec;  // Max rise per second for live peaks
                             // (frame-rate independent attack limit)
};
...
inline constexpr AudioInputParams kParamsM5{
    ...
    1000000, // quietEnvFloorGate (cutoff gates all ssound when below this
             // level)
...
// I2S External microphones (INMP441, etc.) use higher postScale and less
// aggressive gating
inline constexpr AudioInputParams kParamsI2SExternal{
    ...
    0.01f, // energyMinEnv (higher floor to prevent over-normalization
           // with quiet I2S mics)
    1.0f,  // bandCompHigh (no attenuation for high bands)
    3.0f,  // envFloorFromNoise
...
// AudioInputParams instances for different microphone types - used
// directly as template parameters

// PeakData
//
// Simple data class that holds the music peaks for up to NUM_BANDS bands.
// Moved above the ENABLE_AUDIO conditional so both stub and full
// implementations can expose a consistent interface returning PeakData.
...
// SoundAnalyzer
//
// The SoundAnalyzer class uses I2S to read samples from the microphone
// and then runs an FFT on the results to generate the peaks in each band,
// as well as tracking an overall VU and VU ratio, the latter being the
// ratio of the current VU to the trailing min and max VU.
...
public:
    ...
    // I'm old enough I can only hear up to about 12000Hz, but feel free
    // to adjust.  Remember from school that you need to sample at double
    // the frequency you want to process, so 24000 samples is 12000Hz
    ...
    float _energyMaxEnv = 0.01f; // adaptive envelope for autoscaling
                                 // (start low for fast adaptation)
    std::array<float, NUM_BANDS>
        _noiseFloor{};           // adaptive per-band noise floor
    std::array<float, NUM_BANDS>
        _rawPrev{}; // previous raw (noise-subtracted) power for smoothing
    ...
    // Compile-time microphone parameters - bind to template param by
    // reference
    static constexpr const AudioInputParams &_params = Params;

    // Energy spectrum processing (implemented inline below)
    //
    // Calculate a logarithmic scale for the bands like you would find on
    // a graphic equalizer display
    //

private:
    static constexpr int kBandOffset =
        2; // number of lowest source bands to skip in layout (skip bins
           // 0,1,2)
    ...
    PeakData _Peaks; // cached last normalized peaks (moved earlier for
                     // inline method visibility)
    ...
    void FFT()
    {
        ArduinoFFT<float> _FFT(_vReal.data(), _vImaginary.data(),
                               MAX_SAMPLES, SAMPLING_FREQUENCY);
        _FFT.dcRemoval();
    ...
    void SampleAudio()
    {
        ...
        constexpr auto bytesExpected =
            MAX_SAMPLES * sizeof(ptrSampleBuffer[0]);
        if (M5.Mic.record((int16_t *)ptrSampleBuffer.get(), MAX_SAMPLES,
        ...
        // External I2S microphones like INMP441 produce 24-bit samples in
        // 32-bit words. Read stereo frames (RIGHT_LEFT) and auto-select
        // whichever channel carries the mic.
        {
            constexpr int kChannels    = 2; // RIGHT + LEFT
            constexpr auto wordsToRead = MAX_SAMPLES * kChannels;
            constexpr auto bytesExpected32 =
                wordsToRead * sizeof(int32_t);
            static int32_t raw32[wordsToRead];
            // I2S is already running continuously; just read from DMA
            // buffers
            ESP_ERROR_CHECK(i2s_read(I2S_NUM_0, (void *)raw32,
                                     bytesExpected32, &bytesRead,
            ...
            // Downconvert 24-bit left-justified samples from the chosen
            // channel to signed 16-bit INMP441 produces 24-bit samples
            // left-justified in 32-bit words (bits [31:8]) For better
            // dynamic range, we shift by 16 to get the top 16 bits of the
            // 24-bit sample Apply additional scaling since INMP441 may
            // have lower sensitivity than expected
            for (int i = 0; i < MAX_SAMPLES; i++)
            {
                int32_t v = raw32[i * kChannels + s_chanIndex];
                // Take top 16 bits and apply 2x scaling for INMP441
                // sensitivity compensation
                int32_t scaled =
                    (v >> 15); // Shift by 15 instead of 16 for 2x gain
                ptrSampleBuffer[i] = (int16_t)std::clamp(
                    scaled, (int32_t)INT16_MIN, (int32_t)INT16_MAX);
            }
        }
#else
        // ADC or generic 16-bit I2S sources
        {
            constexpr auto bytesExpected16 =
                MAX_SAMPLES * sizeof(ptrSampleBuffer[0]);
            // I2S is already running continuously; just read from DMA
            // buffers
            ESP_ERROR_CHECK(i2s_read(
                I2S_NUM_0, (void *)ptrSampleBuffer.get(), bytesExpected16,
                &bytesRead, 100 / portTICK_PERIOD_MS));
            if (bytesRead != bytesExpected16)
            {
                debugW(
                    "Could only read %u bytes of %u in FillBufferI2S()\n",
    ...
    void UpdateVU(float newval)
    {
        ...
        if (_VU > _PeakVU)
            ...
            _PeakVU =
                (_oldPeakVU * VUDAMPENMAX + _VU) / (VUDAMPENMAX + 1);
        _oldPeakVU = _PeakVU;
        if (_VU < _MinVU)
            _MinVU = _VU;
        else
    ...
    // Compute the band layout based on the sampling frequency and number
    // of bands

    // This computes the start and end bins for each band based on the
    // sampling frequency, ensuring that the bands are spaced
    // logarithmically or in Mel scale as configured. The results are
    // stored in _bandBinStart and _bandBinEnd arrays.

    void ComputeBandLayout()
    {
        const float fMin = LOWEST_FREQ;
        const float fMax =
            std::min<float>(HIGHEST_FREQ, SAMPLING_FREQUENCY / 2.0f);
        const float binWidth =
            (float)SAMPLING_FREQUENCY / (MAX_SAMPLES / 2.0f);
        int prevBin = 4; // Start at bin 4 to skip DC (bin 0), very low
                         // freq (bins 1-3)
        ...
        {
            // Shift the effective band index by kBandOffset so logical
            // band 0 starts higher
            const int logicalIdx = b + kBandOffset;
            const float fracHi   = (float)(logicalIdx + 1) /
                                 (float)(NUM_BANDS + kBandOffset);
#if SPECTRUM_BAND_SCALE_MEL
            ...
            hiBin            = std::clamp(hiBin, prevBin + 1,
                                          (int)(MAX_SAMPLES / 2 - 1));
    ...
    {
        // Band offset handled in ComputeBandLayout so index 0 is the
        // lowest VISIBLE band
        float frameMax    = 0.0f;
        ...
        const float kDisplayGain =
            _params.postScale; // Use postScale from AudioInputParams
        ...
        // Use reciprocal of AudioFPS for frame-rate independent attack
        // limiting
        ...
        for (int b = 0; b < NUM_BANDS; b++)
        {
            ...
            float avgPower =
                (widthBins > 0) ? (sumPower / (float)widthBins) : 0.0f;
            avgPower *= _params.windowPowerCorrection;

            // Apply aggressive logarithmic bass suppression with steeper
            // initial curve
            float bandRatio =
                (float)b / (NUM_BANDS - 1); // 0.0 to 1.0 across all bands

            // Two-stage suppression: moderate suppression on first few
            // bands, then quick recovery
            float suppression;
            if (bandRatio < 0.1f)
            { // First 10% of bands get moderate suppression
                // Quadratic suppression for the very first bands (band 0
                // gets maximum suppression)
                float localRatio =
                    bandRatio /
                    0.1f; // 0.0 to 1.0 over first 10% of spectrum
                suppression =
                    0.005f +
                    (0.05f - 0.005f) *
                        (localRatio * localRatio); // 0.005 to 0.05 (99.5%
                                                   // to 95% attenuation)
            }
            else if (bandRatio < 0.4f)
            { // Next 30% get moderate suppression with quick recovery
                // Exponential recovery from 0.05 to bandCompHigh over
                // bands 10-40%
                float localRatio =
                    (bandRatio - 0.1f) /
                    0.3f; // 0.0 to 1.0 over 10-40% of spectrum
                suppression =
                    0.05f *
                    expf(localRatio * logf(_params.bandCompHigh / 0.05f));
            }
            else
            {
            ...
            if (avgPower > _noiseFloor[b])
                _noiseFloor[b] =
                    _noiseFloor[b] * (1.0f - _params.energyNoiseAdapt) +
                    (float)avgPower * _params.energyNoiseAdapt;
            else
        ...
        // Manually clamp back the bass bands, as they are vastly
        // over-represented
        ...
        if (frameMax > _energyMaxEnv)
            ...
            _energyMaxEnv =
                std::max(_params.energyMinEnv,
                         _energyMaxEnv * _params.energyEnvDecay);

        // Raw SNR-based frame gate (pre-normalization) to suppress steady
        // HVAC and similar backgrounfd noises
        ...
        // Anchor normalization to noise-derived floor to avoid auto-gain
        // blow-up
        float noiseMean = noiseSum / (float)NUM_BANDS;
        float envFloorRaw =
            noiseMean *
            _params.envFloorFromNoise; // raw (unclamped) env floor
        // Quiet environment gate: if the derived env floor is below a
        // configured threshold, suppress the whole frame
        if (_params.quietEnvFloorGate > 0.0f &&
            envFloorRaw < _params.quietEnvFloorGate)
        {
            for (int b = 0; b < NUM_BANDS; ++b)
            {
                _vPeaks[b] = 0.0f;
        ...
        // Now that layout skips the lowest bins, emit all NUM_BANDS
        // directly with no reindexing
        for (int b = 0; b < NUM_BANDS; b++)
        {
            float vTarget = _vPeaks[b] * invEnv;
            vTarget =
                powf(std::max(0.0f, vTarget), _params.compressGamma);
            if (vTarget > 1.0f)
                vTarget = 1.0f;
            vTarget *= kDisplayGain;
            if (vTarget > 1.0f)
                vTarget = 1.0f;
            const float bandFloor =
                std::max(kBandFloorMin, kBandFloorScale);
...
public:
    ...
    // Average normalized energy this frame (0..1 after
    // gating/compression). Updated in
    // ProcessPeaksEnergy()/SetPeakDataFromRemote via UpdateVU().
    ...
    inline bool IsRemoteAudioActive() const override
    {
        return millis() - _msLastRemoteAudio <= AUDIO_PEAK_REMOTE_TIMEOUT;
    }

    // Returns the latest per-band normalized peaks (0..1).
    // Pointer remains valid until next
    // ProcessPeaksEnergy/SetPeakDataFromRemote.
    ...
    // Construct analyzer, allocate buffers (PSRAM-preferred), set initial
    // state. Throws std::runtime_error on allocation failure. Computes
    // band layout once.

    SoundAnalyzer()
    {
        ptrSampleBuffer.reset((int16_t *)heap_caps_malloc(
            MAX_SAMPLES * sizeof(int16_t), MALLOC_CAP_8BIT));
        if (!ptrSampleBuffer)
...
// Stop I2S if it was started
    ...
    // These functions allow access to the last-acquired sample buffer and
    // its size so that effects can draw the waveform or do other things
    // with the raw audio data
    ...
    // BeatEnhance
    //
    // Looks like pure voodoo, but it returns the multiplier by which to
    // scale a value to enhance it by the current VURatioFade amount.  The
    // amt amount is the amount of your factor that should be made up of
    // the VURatioFade multiplier. So passing a 0.75 is a lot of beat
    // enhancement, whereas 0.25 is a little bit.

    // Compute a blend factor using VURatioFade to "pulse" visuals.
    // amt in [0..1] controls how strongly the ratio influences the
    // result.
    ...
    void SampleBufferInitI2S()
    {
        ...
        // Can't use speaker and mic at the same time, and speaker
        // defaults on, so turn it off
        ...
        // This block is for TTGO, MESMERIZER, SPECTRUM_WROVER_KIT and
        // other projects that use an analog mic connected to the input
        // pin.

        static_assert(
            SOC_I2S_SUPPORTS_ADC,
            "This ESP32 model does not support ADC built-in mode");
    ...
    // Called once per frame; uses AppTime.LastFrameTime() and
    // configurable rates.

    inline void DecayPeaks()
    {
        // Use reciprocal of AudioFPS for frame-rate independent decay
        // timing
        float audioFrameTime =
            (_AudioFPS > 0) ? (1.0f / (float)_AudioFPS) : 0.016f;
        if (audioFrameTime <= 0.0f || audioFrameTime > 0.1f)
            audioFrameTime = 0.016f;

        float decayAmount1 =
            std::max(0.0f, audioFrameTime * _peak1DecayRate);
        float decayAmount2 =
            std::max(0.0f, audioFrameTime * _peak2DecayRate);
        ...
        // Removed old smoothing of decay overlays; smoothing now applies
        // to live peaks in ProcessPeaksEnergy()
    }

    // Update the per-band decay overlays from the latest peaks.
    // Rises are limited by VU_REACTIVITY_RATIO; records timestamps on new
    // primary peaks.
    ...
    inline void UpdatePeakData()
    {
        for (int i = 0; i < NUM_BANDS; i++)
        {
            ...
            {
                const float maxIncrease = std::max(
                    0.0, g_Values.AppTime.LastFrameTime() *
                             _peak1DecayRate * VU_REACTIVITY_RATIO);
                _peak1Decay[i] =
                    std::min(_Peaks[i], _peak1Decay[i] + maxIncrease);
            ...
            if (_Peaks[i] > _peak2Decay[i])
            {
                const float maxIncrease = std::max(
                    0.0, g_Values.AppTime.LastFrameTime() *
                             _peak2DecayRate * VU_REACTIVITY_RATIO);
                _peak2Decay[i] =
                    std::min(_Peaks[i], _peak2Decay[i] + maxIncrease);
            }
        }
    }

    // Accept externally provided peaks (e.g., over WiFi) and update
    // internal state. Also recomputes VU from the new band values and
    // records source time.
    ...
    // Uses local mic if no recent remote peaks; otherwise trusts remote
    // and only updates VU.
...
// Project-specific SoundAnalyzer type selection using AudioInputParams
// directly
#if ENABLE_AUDIO
#if M5STICKCPLUS2
