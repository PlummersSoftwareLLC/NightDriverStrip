...
{
public:
    FanBeatEffect(const String &strName) : EffectWithId(strName) {}

    FanBeatEffect(const JsonObjectConst &jsonObject) : EffectWithId(jsonObject) {}
...
public:
    TapeReelEffect(const String &strName) : EffectWithId(strName) {}
    TapeReelEffect(const JsonObjectConst &jsonObject) : EffectWithId(jsonObject) {}
...
public:
    PaletteReelEffect(const String &strName) : EffectWithId(strName) {}

    PaletteReelEffect(const JsonObjectConst &jsonObject) : EffectWithId(jsonObject) {}
    ...
    void DrawEffect()
    {
        for (int i = 0; i < NUM_FANS; i++)
        {
            if (ReelDir[i] != 0)
            {
                ...
                {
                    DrawFanPixels(i * FAN_SIZE + ((pos + x) % FAN_SIZE), 1,
                                  ColorFromPalette(RainbowColors_p, ColorOffset[i] + x * 4, 255, NOBLEND));
                }
            }
        }
    }
...
public:
    PaletteSpinEffect(const String &strName, const CRGBPalette16 &palette, bool bReplace, float sparkleChance = 0.0) :
        EffectWithId(strName), _Palette(palette), _bReplaceMagenta(bReplace), _sparkleChance(sparkleChance)
    {
    }
...
public:
    ColorCycleEffect(PixelOrder order = Sequential, int step = 8) :
        EffectWithId("ColorCylceEffect"), _order(order), _step(step)
    {
    }

    ColorCycleEffect(const JsonObjectConst &jsonObject) :
        EffectWithId(jsonObject), _order((PixelOrder)jsonObject[PTY_ORDER]), _step(jsonObject["stp"])
    {
    }
...
class FireFanEffect : public EffectWithId<FireFanEffect>
{
protected:
...
public:
    FireFanEffect(CRGBPalette16 palette, int ledCount, int cellsPerLED = 1, float cooling = 20, uint8_t sparking = 100,
                  int sparks = 3, int sparkHeight = 4, PixelOrder order = Sequential, bool breversed = false,
                  bool bmirrored = false, bool bmulticolor = false, uint8_t maxSparkTemp = 255) :
        EffectWithId("FireFanEffect"),
        Palette(palette),
        LEDCount(ledCount),
    ...
    virtual void DrawFire(PixelOrder order = Sequential)
    {
        ...
        EVERY_N_MILLISECONDS(20)
        {
            ...
            for (int i = 0; i < CellCount(); i++)
                abHeat[i] = min(255, (abHeat[i] * BlendSelf + abHeat[(i + 1) % CellCount()] * BlendNeighbor1 +
...
public:
    ...
    RingTestEffect() : EffectWithId("Ring Test") {}

    RingTestEffect(const JsonObjectConst &jsonObject) : EffectWithId(jsonObject) {}
...
public:
    void Draw()
    {
        ...
        // Draw four outer pixels in second ring outwards.  We draw 1.05 to take advantage of the non-linear red
        // response in the second pixels (when drawn at 5%, the red will show up more, depending on color correction).
...
public:
    LanternEffect() : EffectWithId("LanternEffect") {}

    LanternEffect(const JsonObjectConst &jsonObject) : EffectWithId(jsonObject) {}
