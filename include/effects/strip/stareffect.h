...
// Description:
//
//   An effect that twinkles stars in and out.  Adapted from my
//   NightDriver code.
//
...
public:
    ...
    Star(const CRGBPalette16 &palette, TBlendType blendType = NOBLEND,
         float maxSpeed = 1.0, float starSize = 1.0) :
...
class RandomPaletteColorStar :
    public MovingFadingPaletteObject,
    public ObjectSize
{
public:
    virtual float GetStarSize()
    ...
    RandomPaletteColorStar(const CRGBPalette16 &palette,
...
public:
    ...
    LongLifeSparkleStar(const CRGBPalette16 &palette,
                        TBlendType blendType = NOBLEND,
                        float maxSpeed = 1.0, float starSize = 1.0) :
        MovingFadingPaletteObject(palette, blendType, maxSpeed),
        ObjectSize(starSize)
...
public:
    ...
    ColorStar(CRGB color, float maxSpeed = 1.0, float starSize = 1.0) :
        MovingFadingColoredObject(color, maxSpeed), ObjectSize(starSize)
    {
    }
};
...
{
public:
    QuietStar(const CRGBPalette16 &palette,
              TBlendType blendType = NOBLEND, float maxSpeed = 10.0,
              float starSize = 1) :
...
{
public:
    MusicStar(const CRGBPalette16 &palette,
              TBlendType blendType = NOBLEND, float maxSpeed = 2.0,
              float starSize = 1) :
...
{
public:
    MusicPulseStar(const CRGBPalette16 &palette,
                   TBlendType blendType = LINEARBLEND,
                   float maxSpeed = 0.0, float size = 0.0) :
    ...
    virtual float GetStarSize() const
    {
        return 1 + _objectSize * g_Analyzer.VURatio();
    }
};
...
public:
    BubblyStar(const CRGBPalette16 &palette,
               TBlendType blendType = LINEARBLEND, float maxSpeed = 2.0,
               float starSize = 12) :
        Star(palette, blendType, maxSpeed, starSize)
    {
...
public:
    ColorCycleStar(const CRGBPalette16 &palette,
                   TBlendType blendType = LINEARBLEND,
                   float maxSpeed = 2.0, int speedDivisor = 1) :
    ...
    {
        CRGB c = ColorFromPalette(_palette, millis() / 2048.0f,
                                  _brightness, blend);
...
public:
    MultiColorStar(const CRGBPalette16 &palette,
                   TBlendType blendType = LINEARBLEND,
                   float maxSpeed = 2.0, int speedDivisor = 1) :
        Star(palette, blendType, maxSpeed)
    {
...
{
public:
    ChristmasLightStar(const CRGBPalette16 &palette, TBlendType blendType,
                       float maxSpeed = 0.0) :
...
{
public:
    HotWhiteStar(const CRGBPalette16 &palette,
                 TBlendType blendType = LINEARBLEND, float maxSpeed = 0.0,
                 float size = 0.0) :
        Star(palette, blendType, maxSpeed, size)
    {
    }
    ...
    virtual CRGB RenderColor(TBlendType blend)
    {
        if (Age() < IgnitionTime() + HoldTime())
            return CRGB::White;
        CRGB c =
            ColorFromPalette(_palette, 130 * (1.0f - FadeoutAmount()),
                             256 * (1.0f - FadeoutAmount()), blend);
        return c;
    }
};

/*
template <typename ObjectType> class BeatStarterEffect : public
BeatEffectBase
{
  protected:
  ...
  public:
    ...
    {
        ObjectType newstar(_palette, _blendType, _maxSpeed * _musicFactor,
_starSize);
        // This always starts stars on even pixel boundaries so they look
like the desired width if not moving newstar._iPos = (int) random_range(0,
_cLEDs-1-starWidth); _allParticles.push_back(newstar);
...
class StarEffectBase :
    public EffectWithId<StarEffectBase<StarType, TEffect>>
{
protected:
...
public:
    StarEffectBase(const String &strName, const CRGBPalette16 &palette,
                   float probability = 1.0, float starSize = 1.0,
                   TBlendType blendType = LINEARBLEND,
                   float maxSpeed = 100.0, float blurFactor = 0.0,
                   float musicFactor = 1.0, CRGB skyColor = CRGB::Black) :
    ...
    virtual void Clear()
    {
        LEDStripEffect::setAllOnAllChannels(_skyColor.r, _skyColor.g,
                                            _skyColor.b);
    }
    ...
    virtual void CreateStars()
    {
        ...
        for (int i = 0; i < cMaxNewStarsPerFrame; i++)
        {
            ...
            prob = (prob / 100) +
                   (g_Analyzer.VURatio() - 1.0) * _musicFactor;

            constexpr auto kProbabilitySpan = 1.0;

            if (g_Analyzer.VU() > 0)
            {
                if (random_range(0.0, kProbabilitySpan) <
                    g_Values.AppTime.LastFrameTime() * prob)
                {
                    StarType newstar(_palette, _blendType,
                                     _maxSpeed * _musicFactor, _starSize);
                    // This always starts stars on even pixel boundaries
                    // so they look like the desired width if not moving
                    newstar._iPos = (int)random_range(
                        0U, LEDStripEffect::_cLEDs - 1 - starWidth);
    ...
    virtual void Update()
    {
        // Any particles that have lived their lifespan can be removed.
        // They should be found at the back of the array
        while (_allParticles.size() > 0 &&
               (_allParticles.front().Age() >=
                _allParticles.front().TotalLifetime()))
            _allParticles.pop_front();

        // If we've created too many stars, prune the newest first, before
        // they get painted
    ...
    void Draw() override
    {
        ...
        {
            LEDStripEffect::g()->blurRows(LEDStripEffect::g()->leds,
                                          MATRIX_WIDTH, MATRIX_HEIGHT, 0,
                                          _blurFactor * 255);
            LEDStripEffect::fadeAllChannelsToBlackBy(
                55 * (2.0 - g_Analyzer.VURatioFade()));
        }
             ...
             i++)
        {
            ...
            LEDStripEffect::setPixelsFOnAllChannels(
                fPos - i->_objectSize / 2.0, i->_objectSize, c, true);
        }
    }
};
...
template <typename StarType>
class BlurStarEffect :
    public StarEffectBase<StarType, BlurStarEffect<StarType>>
{
public:
    BlurStarEffect(const CRGBPalette16 &palette, float probability = 0.2,
                   size_t starSize      = 1,
                   TBlendType blendType = LINEARBLEND,
                   float maxSpeed       = 20.0) :
        StarEffectBase<StarType, BlurStarEffect<StarType>>(
            palette, probability, starSize, blendType, maxSpeed)
    {
    }
...
public:
    ...
    TwinkleStarEffect(const JsonObjectConst &jsonObject) :
        EffectWithId<TwinkleStarEffect>(jsonObject)
    {
    }
