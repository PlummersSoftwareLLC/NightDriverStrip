...
// Description:
//
//    Floating point framerate independent version of the classic Flame
//    effect
//
...
// BeatEffectBase
//
// A specialization of LEDStripEffect, adds a HandleBeat function that
// allows apps to draw based on the music beat.  The Draw() function does
// the audio processing and calls HandleBeat() whenever.  Apps are free to
// draw in both Draw() and HandleBeat().
//
// The constructor allows you to specify the sensitivity by where the
// latch points are/ For a highly sensitive (defaults), keep them both
// close to 1.0.  For a wider beat detection you could use 0.25 and 1.75
// for example.
...
public:
    BeatEffectBase(float minRange = 0.75, float minElapsed = 0.20) :
        _minRange(minRange), _minElapsed(minElapsed)
    {
    }

    // When a beat is detected, this is called.  The 'bMajor' indicates
    // whether this is a more important beat, which for now simply means
    // it's been a minimum delay since the last beat.
    ...
    // BeatEffectBase::Draw
    //
    // Doesn't actually "draw" anything, but rather it scans the audio VU
    // to detect beats, and when it finds one, it calls the virtual
    // "HandleBeat" function.
    ...
    virtual void ProcessAudio()
    {
        ...
        // Serial.printf("Samples: %d, max: %0.2f, min: %0.2f, span:
        ...
        if (maximum - minimum > _minRange)
        {
            ...
            {
                // Serial.printf("False Beat: elapsed: %0.2f, range:
                // %0.2f, time: %0.2lf\n", elapsed, maximum - minimum,
                // g_AppTime.CurrentTime());
                //  False beat too early, clear data but don't reset
                //  lastBeat
                _samples.clear();
            }
            else
...
//
// Uses a very sensitive beat detection.  Fills all pixels blue based on
// VU, and on beats, fills a random insulator with a random color.  Longer
// beats get more insulators.  Very long beats get everything filled with
// purple.

class SimpleColorBeat :
    public BeatEffectBase,
    public EffectWithId<SimpleColorBeat>
{
protected:
    ...
    void Draw() override
    {
        ...
        CRGB c = CRGB::Blue * g_Analyzer.VURatio() *
                 g_Values.AppTime.LastFrameTime() * 0.75;
    ...
    void HandleBeat(bool bMajor, float elapsed, float span) override
    {
        ...
        else
        {
            ...
            if (elapsed >
                0.5) // Medium beats fill blue and proceed with insulator2
            {
        ...
        for (int iPass = 0; iPass < cInsulators; iPass++)
        {
            ...
            { // Pick a different insulator than was used last time by:
                i = random(0,
                           NUM_FANS); //  - Starting with a random number
            } while (i ==
                     _iLastInsulator); //  - Repeating until it doesn't
                                       //  match the last pass
            _iLastInsulator = i; // Our current choice forms the new
                                 // "last" choice for next pass

            DrawFanPixels(0, FAN_SIZE, c, Sequential,
                          i);    // Draw twice to float-saturate our color
...
public:
    ...
    SimpleColorBeat(const JsonObjectConst &jsonObject) :
        BeatEffectBase(0.5, 0.25),
        EffectWithId<SimpleColorBeat>(jsonObject)
    {
    }
};
