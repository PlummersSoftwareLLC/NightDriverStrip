...
public:
    ...
    virtual void Init(std::shared_ptr<GFXBase> pGFX, size_t meteors = 4,
                      int size = 4, int decay = 3, float minSpeed = 0.5,
                      float maxSpeed = 0.5)
    {
        meteorCount      = meteors;
        meteorSize       = size;
        ...
        for (int i = 0; i < meteorCount; i++)
        {
            ...
            hueval %= 256;
            hue[i]   = hueval;
            iPos[i]  = meteorCount <= 1
                           ? 0
                           : (pGFX->GetLEDCount() / (meteorCount - 1) * i);
    ...
    virtual void Draw(LEDStripEffect *owner)
    {
        ...
        for (int j = 0; j < pGFX->GetLEDCount(); j++)
        {
            ...
            {
                owner->fadePixelToBlackOnAllChannelsBy(j,
                                                       meteorTrailDecay);
            }
        }
        ...
        for (int i = 0; i < meteorCount; i++)
        {
            ...
            for (int j = 0; j < meteorSize; j++)
            {
                ...
                if ((x <= static_cast<int>(pGFX->GetLEDCount())) &&
                    (x >= 1))
                {
                    CRGB rgb;
                    hue[i] = hue[i] + 0.025f;
                    ...
                    // Blend with current pixel from primary device, then
                    // set on all channels
...
public:
    MeteorEffect(int cMeteors = 4, uint size = 4, uint decay = 3,
                 float minSpeed = 0.2, float maxSpeed = 0.2) :
    ...
    bool Init(std::vector<std::shared_ptr<GFXBase>> &gfx) override
    {
        ...
        for (auto &device : gfx)
        {
            ...
            channel.Init(device, _cMeteors, _meteorSize,
                         _meteorTrailDecay, _meteorSpeedMin,
                         _meteorSpeedMax);
