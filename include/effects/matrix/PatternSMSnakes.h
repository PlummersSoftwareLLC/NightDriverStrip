...
#if ENABLE_AUDIO
class PatternSMSnakes : public BeatEffectBase, public LEDStripEffect
#else
...
private:
  ...
  const int SNAKES_LENGTH = (8U); // длина червяка от 2 до 15 (+ 1 пиксель голова/хвостик), ограничена размером переменной для хранения трактории тела червяка
  static constexpr int trackingOBJECT_MAX_COUNT = (100U);  // максимальное количество отслеживаемых объектов (очень влияет на расход памяти)
  ...
  static constexpr int enlargedOBJECT_MAX_COUNT = MATRIX_WIDTH * 2; // максимальное количество сложных отслеживаемых объектов (меньше, чем trackingOBJECT_MAX_COUNT)
...
public:
  ...
  for (uint8_t i = 0; i < enlargedObjectNUM; i++){
   ...
   if (trackingObjectSpeedY[i] >= 1)
   {
    ...
    g()->leds[g()->xy(x,y)] += CHSV(trackingObjectHue[i], 255U, trackingObjectSpeedY[i] * 255); // тут рисуется голова
    ...
    for (uint8_t m = 0; m < SNAKES_LENGTH; m++){ // 16 бит распаковываем, 14 ещё остаётся без дела в запасе, 2 на хвостик
      ...
      g()->leds[g()->xy(x,y)] += CHSV(trackingObjectHue[i] + (m + trackingObjectSpeedY[i])*4U, 255U, 255U); // тут рисуется тело
    ...
    x = (WIDTH + x + dx) % WIDTH;
    y = (HEIGHT + y + dy) % HEIGHT;
    g()->leds[g()->xy(x,y)] += CHSV(trackingObjectHue[i] + (SNAKES_LENGTH + trackingObjectSpeedY[i])*4U, 255U, (1 - trackingObjectSpeedY[i]) * 255); // хвостик
  }
}
  ...
  virtual void HandleBeat(bool bMajor, float elapsed, float span) override
  {
    ...
    // the paramaters at runtine just makes the code explode. Settle for changing
    // the colors and speeds.
    for (uint8_t i = 0; i < enlargedObjectNUM; i++){
      trackingObjectHue[i] = random8();
    }
