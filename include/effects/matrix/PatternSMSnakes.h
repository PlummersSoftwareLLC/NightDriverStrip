...
#endif
{
  private:
    ...
    static constexpr int enlargedOBJECT_MAX_COUNT = MATRIX_WIDTH * 2; // максимальное количество сложных отслеживаемых
  ...
  public:
    ...
    void Draw() override
    {
        ...
        for (uint8_t i = 0; i < enlargedObjectNUM; i++)
        {
                ...
                if (random8(9U) == 0U) // вероятность поворота
                    ...
                    { // -> поворот направо
                        enlargedObjectTime[i] =
                            (enlargedObjectTime[i] << 2) | B11; // младший бит = поворот, старший = направо
                        switch (trackingObjectState[i])
                        {
                        case B11:
                            trackingObjectState[i] = B01;
            ...
            g()->leds[XY(x, y)] += CHSV(trackingObjectHue[i], 255U,
                                        trackingObjectSpeedY[i] * 255); // тут рисуется голова

            for (uint8_t m = 0; m < SNAKES_LENGTH; m++)
            { // 16 бит распаковываем, 14 ещё остаётся без дела в запасе, 2
              // на хвостик
                x = (WIDTH + x + dx) % WIDTH;
                ...
                // тут рисуется тело
                g()->leds[XY(x, y)] +=
                    CHSV(trackingObjectHue[i] + (m + trackingObjectSpeedY[i]) * 4U, 255U, 255U); // тут рисуется тело
            ...
            x = (WIDTH + x + dx) % WIDTH;
            y = (HEIGHT + y + dy) % HEIGHT;
            g()->leds[XY(x, y)] += CHSV(trackingObjectHue[i] + (SNAKES_LENGTH + trackingObjectSpeedY[i]) * 4U, 255U,
                                        (1 - trackingObjectSpeedY[i]) * 255); // хвостик
        }
    }
