...
void addGlitter(uint8_t chanceOfGlitter){
  if ( random8() < chanceOfGlitter) g()->leds[random16(NUM_LEDS)] = random(0, 16777215);
}
...
void spruce() {
  ...
  if (!(WIDTH& 0x01))
    g()->leds[g()->xy(WIDTH/2 - ((millis()>>9) & 0x01 ? 1:0), minDim - 1 - ((millis()>>8) & 0x01 ? 1:0))] = CHSV(0, 255, 255);
  else
    g()->leds[g()->xy(WIDTH/2, minDim - 1)] = CHSV(0, (millis()>>9) & 0x01 ? 0 : 255, 255);
