...
private:
    ...
    const uint8_t width_adj =
        (MATRIX_WIDTH < MATRIX_HEIGHT ? (MATRIX_HEIGHT - MATRIX_WIDTH) / 2
                                      : 0);
    const uint8_t height_adj =
        (MATRIX_HEIGHT < MATRIX_WIDTH ? (MATRIX_WIDTH - MATRIX_HEIGHT) / 2
                                      : 0);
    ...
    void confetti()
    {
        ...
        for (unsigned i = 0; i < scaleToNumLeds; i++)
            if (random8() < density)
                if ((g()->getPixel(idx).r + g()->getPixel(idx).g +
                     g()->getPixel(idx).b) < 10)
    ...
    void spruce()
    {
        hue++; // Increment the hue value, which likely controls the color
               // of the LEDs.

        // Fade all LED channels to black based on the 'speed' value.
        // The 'map' function scales the 'speed' value from one range to
        // another.
        fadeAllChannelsToBlackBy(::map(speed, 1, 255, 1, 100));

        uint8_t z;
        if (effId == 3)
            z = triwave8(hue); // Set 'z' to a value calculated using
                               // 'triwave8' if 'effId' is 3.
        else
            z = beatsin8(1, 1, 255); // Otherwise, set 'z' to a value
                                     // calculated using 'beatsin8'.

        for (uint8_t i = 0; i < minDim; i++)
        {
            // Calculate 'x' based on various factors.
            unsigned x = beatsin16(i * (::map(speed, 1, 255, 3, 20)),
                                   i * 2, (minDim * 4 - 2) - (i * 2 + 2));

            if (effId == 2)
            {
                // Draw a pixel with certain conditions if 'effId' is 2.
                g()->drawPixelXYF_Wu(
                    x / 4 + height_adj, (float)(MATRIX_HEIGHT - 1 - i),
                    random8(10) == 0
            ...
            {
                // Draw a pixel with different color conditions if 'effId'
                // is not 2.
                g()->drawPixelXYF_Wu(x / 4 + height_adj,
                                     (float)(MATRIX_HEIGHT - 1 - i),
                                     CHSV(hue + i * z, 255, 255));
            }
        }
        ...
        {
            g()->leds[XY(MATRIX_WIDTH / 2, minDim - 1)] =
...
public:
    PatternSMHolidayLights() :
        EffectWithId<PatternSMHolidayLights>("Tannenbaum")
    {
    }
    PatternSMHolidayLights(const JsonObjectConst &jsonObject) :
        EffectWithId<PatternSMHolidayLights>(jsonObject)
    {
    }
