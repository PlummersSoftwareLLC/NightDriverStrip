...
#if USE_AUDIO
class PatternSMTraffic : public BeatEffectBase, public LEDStripEffect
#else
...
private:
  ...
  static constexpr uint32_t colors[4][4] ={
    // light |atr_light |stop_light|atr_stop
...
// функция отрисовки точки по координатам X Y
  ...
  void Draw() override
  {
      ...
      if (hue == 1) {
        ...
        color = ((random8(2)) == 1 ? CRGB::Black : (random8(2)) == 1 ? (random8(20) == 5 ? color_force : atr_stop) : stop_light);
        drawPixelXY((center + 1), 0, color);

        // [T] ****
        color_force = (divider == 0) ? CRGB::Blue : atr_light;
        color = ((random8(2)) == 1 ? CRGB::Black : (random8(2)) == 1 ? (random8(20) == 5 ? color_force : atr_light) : light);
        ...
        // [L] ****
        color = ((random8(2)) == 1 ? CRGB::Black : (random8(2)) == 1 ? (random8(20) == 5 ? CRGB::Blue : atr_light) : light);
      ...
      // *********
      // LOG.printf_P(PSTR("Trafic | hue = %03d | divider = %01d | step = %03d\n\r"), hue, divider,  step);
      // -------------------------------------
      step++;
  }
#if USE_AUDIO
