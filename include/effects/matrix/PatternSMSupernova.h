...
#if ENABLE_AUDIO
class PatternSMSupernova : public BeatEffectBase, public LEDStripEffect
#else
...
private:
  ...
  const uint8_t CENTER_X_MINOR = (MATRIX_WIDTH / 2) - ((MATRIX_WIDTH - 1) & 0x01); // the center of the matrix according to ICSU, shifted to the smaller side, if the width is even
  const uint8_t CENTER_Y_MINOR = (MATRIX_HEIGHT / 2) - ((MATRIX_HEIGHT - 1) & 0x01); // center of the YGREK matrix, shifted down if the height is even
  const uint8_t CENTER_X_MAJOR = MATRIX_WIDTH / 2 + (MATRIX_WIDTH % 2); // the center of the matrix according to IKSU, shifted to a larger side, if the width is even
  const uint8_t CENTER_Y_MAJOR = MATRIX_HEIGHT / 2 + (MATRIX_HEIGHT % 2); // center of the YGREK matrix, shifted up if the height is even
  ...
  #define enlargedOBJECT_MAX_COUNT                     (MATRIX_WIDTH * 2) // максимальное количество сложных отслеживаемых объектов (меньше, чем MAX_COUNT)
  uint8_t enlargedObjectNUM;                                       // используемое в эффекте количество объектов
  long    enlargedObjectTime[enlargedOBJECT_MAX_COUNT];
#define MAX_COUNT  (254U)  // максимальное количество отслеживаемых объектов (очень влияет на расход памяти)
...
void particlesUpdate2(uint8_t i){
  ...
  PosX[i] += SpeedX[i];
  PosY[i] += SpeedY[i];
  if(State[i] == 0 || PosX[i] <= -1 || PosX[i] >= MATRIX_WIDTH || PosY[i] <= -1 || PosY[i] >= MATRIX_HEIGHT)
    IsShift[i] = false;
}
...
void starfield2Emit(uint8_t i){
  ...
  PosX[i] = MATRIX_WIDTH * 0.5;//CENTER_X_MINOR;// * RENDERER_RESOLUTION; //  particle->x = source->x;
  PosY[i] = MATRIX_HEIGHT * 0.5;//CENTER_Y_MINOR;// * RENDERER_RESOLUTION; //  // particle->y = source->y;
  SpeedX[i] = (((float)random8()-127.)/512.); // random(_hVar)-_constVel; // particle->vx
  SpeedY[i] = sqrt(0.0626-SpeedX[i]*SpeedX[i]); // sqrt(pow(_constVel,2)-pow(SpeedX[i],2)); // particle->vy зависит от particle->vx - не ошибка
  if(random8(2U)) { SpeedY[i]=-SpeedY[i]; }
...
public:
  ...
  void Draw() override
  {
    ...
    ProcessAudio();
      ...
      step = -1;// deltaValue; //счётчик количества частиц в очереди на зарождение в этом цикле
  g()->DimAll(200);
