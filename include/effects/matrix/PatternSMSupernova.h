...
public:
    PatternSMSupernova() :
        EffectWithId<PatternSMSupernova>("Supernova"),
        hue(0),
        hue2(0),
        step(0)
    {
    }
    PatternSMSupernova(const JsonObjectConst &jsonDebrisItem) :
        EffectWithId<PatternSMSupernova>(jsonDebrisItem)
    {
    }
    ...
    {
        std::for_each(
            _debris_items.begin(), _debris_items.end(),
    ...
    void Draw() override
    {
        ...
        for (auto &debris_item : _debris_items)
        {
            ...
            {
                CRGB baseRGB = ColorFromPalette(
                    g()->IsPalettePaused() ? g()->GetCurrentPalette()
                                           : HeatColors_p,
                    debris_item._hue, 255, LINEARBLEND);
                baseRGB.nscale8(debris_item._state);
                g()->drawPixelXYF_Wu(debris_item._position_x,
                                     debris_item._position_y, baseRGB);
            }
        }
    }
...
private:
    ...
    void inline StarfieldEmit(DebrisItem &debris_item)
    {
        ...
        debris_item._speed_y =
            sqrtf(0.0626f - debris_item._speed_x * debris_item._speed_x);
        if (random8(2U))
