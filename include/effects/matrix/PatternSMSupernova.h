...
class PatternSMSupernova : public LEDStripEffect
{
  ...
  private:
    ...
    void starfield2Emit(uint8_t i)
    {
        ...
        SpeedY[i] = sqrt(0.0626 - SpeedX[i] * SpeedX[i]); // sqrt(pow(_constVel,2)-pow(SpeedX[i],2)); //
  ...
  public:
    PatternSMSupernova() : LEDStripEffect(EFFECT_MATRIX_SMSUPERNOVA, "Supernova")
    {
    }
    ...
    void Start() override
    {
        ...
        if (enlargedObjectNUM > MAX_COUNT)
            enlargedObjectNUM = 200U; // MAX_COUNT;
        deltaValue =
