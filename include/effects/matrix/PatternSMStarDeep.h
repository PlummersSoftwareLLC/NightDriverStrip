...
// Inspired by
// https://editor.soulmatelights.com/gallery/1166-stars-beta-ver The
// original has a bunch of Palette management stuff we just didn't
// implement.
...
private:
    ...
    struct StarData
    {
        ...
        int position;    // delay the start of the star relative to the
                         // counter
    };
    ...
    // Countdown timers for drift movement to avoid expensive modulo
    // operations.
    uint8_t x_drift_countdown;
    uint8_t y_drift_countdown;

    static constexpr int kStarBlender =
        128U; // Adjust the brightness or blending of the stars.
    static constexpr int kCenterDriftSpeed =
        6U;   // Speed of movement of the floating star emergence center.
    ...
    static constexpr uint8_t kDriftMarginY =
        ((MATRIX_HEIGHT - 1) / 2) / 2;

public:
    PatternSMStarDeep() :
        EffectWithId<PatternSMStarDeep>("Star Deep"),
        stars(make_unique_psram<StarData[]>(kMaxStars))
    {
    }
    PatternSMStarDeep(const JsonObjectConst &jsonObject) :
        EffectWithId<PatternSMStarDeep>(jsonObject),
        stars(make_unique_psram<StarData[]>(kMaxStars))
    {
    }

    // Draws a multi-point star.
    // This code can draw outside of the matrix boundaries, but
    // DrawStarLine() is expected to handle clipping.
    void DrawStar(int16_t centerX, int16_t centerY, int16_t outerRadius,
                  int16_t innerRadius, uint8_t numPoints,
                  uint8_t angleOffset, uint8_t colorIndex)
    {
        if (numPoints == 0)
            return;

        const CRGB starColor =
            g()->IsPalettePaused()
                ? g()->ColorFromCurrentPalette(colorIndex)
                : ColorFromPalette(*curPalette, colorIndex);
        ...
        for (uint8_t i = 0; i < numPoints; i++)
        {
            ...
            const int16_t outer_x =
                centerX +
                ((outerRadius * (sin8(outer_angle) - 128.0f)) / 128.0f);
            const int16_t outer_y =
                centerY +
                ((outerRadius * (cos8(outer_angle) - 128.0f)) / 128.0f);

            const uint8_t inner_angle_1 = outer_angle + angle_step / 2;
            const int16_t inner_x_1 =
                centerX +
                ((innerRadius * (sin8(inner_angle_1) - 128.0f)) / 128.0f);
            const int16_t inner_y_1 =
                centerY +
                ((innerRadius * (cos8(inner_angle_1) - 128.0f)) / 128.0f);

            const uint8_t inner_angle_2 = outer_angle - angle_step / 2;
            const int16_t inner_x_2 =
                centerX +
                ((innerRadius * (sin8(inner_angle_2) - 128.0f)) / 128.0f);
            const int16_t inner_y_2 =
                centerY +
                ((innerRadius * (cos8(inner_angle_2) - 128.0f)) / 128.0f);

            DrawStarLine(inner_x_1, inner_y_1, outer_x, outer_y,
                         starColor);
            DrawStarLine(inner_x_2, inner_y_2, outer_x, outer_y,
                         starColor);
        }
    }

    // Custom line drawing function.
    // This implementation is kept for its specific visual characteristics
    // ("funky wrapping corners"), which differ from the standard
    // g()->DrawLine().
    ...
    void Start() override
    {
        ...
        // Set an initial location and movement vector for the animation
        // center.
    ...
    void Draw() override
    {
        ...
        // Update drift center position, bouncing off the edges of the
        // defined drift area.
        ...
        for (uint8_t num = 0; num < nStars; num++)
        {
            if (counter >= stars[num].position)
            {
                ...
                {
                    DrawStar(driftx, drifty, 2 * starSize, starSize,
                             stars[num].corners,
                             kStarBlender + stars[num].color,
                             stars[num].color * 2);
                    stars[num].color++;
                }
                else
