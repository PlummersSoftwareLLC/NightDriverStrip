...
// The original has a bunch of Palette management stuff we just didn't implement.
...
private:
  ...
  // const int bballsMaxNUM = 100U;  // the maximum number of tracked objects (very affects memory consumption)
  #define bballsMaxNUM 100U  // the maximum number of tracked objects (very affects memory consumption)
  uint8_t bballsCOLOR[bballsMaxNUM];  // star color (reusing the Balls effect array)
  uint8_t bballsX[bballsMaxNUM];      // number of corners in the star (reusing the Balls effect array
  int   bballsPos[bballsMaxNUM];      // delay the start of the star relative to the counter (reuse the Balls effect array)
...
public:
  ...
  // Don't use the system g()->DrawLine because this code draws into unsafe places, which clobbers
  // memory and brings down the app. Use our clamped versions instead.
  ...
  virtual void Draw() override
  {
    ...
    // This block of 6 'if's could probably be replaced by a std::clamp...if I could read this junk.
    // This keeps the absolute unit of our drift (xy) and the angle of our drift (sin, cos) in check.
