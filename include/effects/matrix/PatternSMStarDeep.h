...
#define bballsMaxNUM \
  100U  // the maximum number of tracked objects (very affects memory
        // consumption)
  uint8_t
      bballsCOLOR[bballsMaxNUM];  // star color (reusing the Balls effect array)
  ...
  const int CENTER_DRIFT_SPEED =
      6U;  // скорость перемещения плавающего центра возникновения звёзд
  ...
  const uint8_t CENTER_X_MINOR =
      (WIDTH / 2) -
      ((WIDTH - 1) & 0x01);  // центр матрицы по ИКСУ, сдвинутый в меньшую
                             // сторону, если ширина чётная
  const uint8_t CENTER_Y_MINOR =
      (HEIGHT / 2) -
      ((HEIGHT - 1) & 0x01);  // центр матрицы по ИГРЕКУ, сдвинутый в меньшую
                              // сторону, если высота чётная
  const uint8_t CENTER_X_MAJOR =
      WIDTH / 2 + (WIDTH % 2);  // центр матрицы по ИКСУ, сдвинутый в большую
                                // сторону, если ширина чётная
  const uint8_t CENTER_Y_MAJOR =
      HEIGHT / 2 + (HEIGHT % 2);  // центр матрицы по ИГРЕКУ, сдвинутый в
 ...
 public:
  ...
  void Start() override {
    ...
    driftx = random8(
        4, WIDTH - 4);  // set an initial location for the animation center
    drifty = random8(
        4, HEIGHT - 4);  // set an initial location for the animation center

    cangle = (sin8(random(25, 220)) - 128.0) /
             128.0;  // angle of movement for the center of animation gives a
                     // float value between -1 and 1
    sangle = (sin8(random(25, 220)) - 128.0) /
             128.0;  // angle of movement for the center of animation in the y
    ...
    for (uint8_t num = 0; num < bballsNUM; num++) {
      bballsX[num] = random8(
          3, 9);  // pointy = random8(3, 9); // количество углов в звезде
      bballsPos[num] =
          counter + (num << 2) + 1U;  // random8(50);//modes[currentMode].Scale;//random8(50,
  ...
  void Drawstar(int16_t xlocl, int16_t ylocl, int16_t biggy, int16_t little,
                int16_t points, int16_t dangle,
                uint8_t koler)  // random multipoint star
  {
    ...
    for (int i = 0; i < points; i++) {
      ...
      DrawLine(
          xlocl +
              ((little * (sin8(i * radius2 + radius2 / 2 - dangle) - 128.0)) /
               128),
          ylocl +
              ((little * (cos8(i * radius2 + radius2 / 2 - dangle) - 128.0)) /
               128),
  ...
  void Draw() override {
    ...
    if (driftx >
        (WIDTH - spirocenterX / 2U))  // change directin of drift if you get
    ...
    for (uint8_t num = 0; num < bballsNUM; num++) {
      ...
      {
        if (counter - bballsPos[num] <=
            WIDTH + 5U) {  //(counter - ringdelay <= WIDTH + 5) {
          // drawstar(driftx  , drifty, 2 * (counter - ringdelay), (counter -
          // ringdelay), pointy, blender + h, h * 2 + 85);
          Drawstar(driftx, drifty, 2 * (counter - bballsPos[num]),
                   (counter - bballsPos[num]), bballsX[num],
                   STAR_BLENDER + bballsCOLOR[num],
          ...
          bballsPos[num] =
              counter + (bballsNUM << 1) +
              1U;  // random8(50, 99);//modes[currentMode].Scale;//random8(50,
                   // 99); // задержка следующего пуска звезды
        }
      }
    }
