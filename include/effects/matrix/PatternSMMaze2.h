...
#define M_SHIFT_Y !(LED_ROWS % 2)
  ...
  void digMaze(int x, int y) {
    ...
    while (count < 4) {
      ...
      x2 = x1 + dx;
      y2 = y1 + dy;
      if (x2 > 0 && x2 < M_WIDTH && y2 > 0 && y2 < M_HEIGHT && maze[x1][y1] &&
          maze[x2][y2]) {
        maze[x1][y1] = 0;
        maze[x2][y2] = 0;
        x = x2;
 ...
 public:
  ...
  void Draw() override {
    ...
    switch (Lookdir) {
      case 0:
        drawPixelXYF(float(posX - M_SHIFT_X) + (float(SubPos) / 255.),
                     (posY - M_SHIFT_Y), CHSV(0, 0, 255));
        break;
      case 1:
        drawPixelXYF((posX - M_SHIFT_X),
                     float(posY - M_SHIFT_Y) - (float(SubPos) / 255.),
                     CHSV(0, 0, 255));
        break;
      case 2:
        drawPixelXYF(float(posX - M_SHIFT_X) - (float(SubPos) / 255.),
