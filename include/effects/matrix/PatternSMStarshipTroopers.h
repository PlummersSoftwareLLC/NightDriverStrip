...
#if ENABLE_AUDIO
class PatternSMStarshipTroopers : public BeatEffectBase, public LEDStripEffect
#else
...
//   fadeToBlackBy(leds, NUM_LEDS, map(speed, 1, 255, 5, 10));
  switch (dir) {
    case 0: // Up
      for (byte x = 0; x < WIDTH; x++) {
        if (x > WIDTH/2 and random(chance) < DIR_CHARGE) {count++; break;}
        for (float y = 0; y < HEIGHT; y+=speedFactor) {
          getPixel(x, y) = (((int)y == HEIGHT - 1) ? CRGB::Black : getPixel(x, y + 1));
        }
      }
      break;
    case 1: // Up - Right
      for (float x = 0; x < WIDTH; x+=speedFactor) {
        if ((uint8_t)x > WIDTH/2 and random(chance) < DIR_CHARGE) {count++; break;}
        for (byte y = 0; y < HEIGHT; y++) {
          getPixel(x, y) = ((y == HEIGHT - 1 or (int)x == WIDTH - 1) ? CRGB::Black : getPixel(x + 1, y + 1));
        }
      }
      break;
    case 2: // Right
      for (float x = 0; x < WIDTH; x+=speedFactor) {
        ...
        for (byte y = HEIGHT - 1; y > 0; y--) {
          getPixel(x, y) = (((int)x == WIDTH - 1) ? CRGB::Black : getPixel(x + 1, y));
        }
      }
      break;
    case 3: // Down - Right
      for (float x = 0; x < WIDTH; x+=speedFactor) {
        if ((uint8_t)x > WIDTH/2 and random(chance) < DIR_CHARGE) {count++; break;}
        for (byte y = HEIGHT - 1; y > 0; y--) {
          getPixel(x, y) = (((int)x == WIDTH - 1 or y == 0) ? CRGB::Black : getPixel(x + 1, y - 1));
        }
      }
      break;
    case 4: // Down
    ...
    case 5: // Down - Left
      for (float x = WIDTH - 1; x > 0; x-=speedFactor) {
        if (!(uint8_t)x < WIDTH/2 and random(chance) < DIR_CHARGE) {count++; break;}
        for (byte y = HEIGHT - 1; y > 0; y--) {
          getPixel(x, y) = ((y == 0 or (int)x == 0) ? CRGB::Black : getPixel(x - 1, y - 1));
        }
      }
      break;
    case 6: // Left
    ...
    case 7: // Up - Left
      for (float x = WIDTH -1; x >0; x-=speedFactor) {
        if ((uint8_t)x < WIDTH/2 and random(chance) < DIR_CHARGE) {count++; break;}
        for (byte y = HEIGHT - 1; y > 0; y--) {
          getPixel(x, y) = ((y == HEIGHT - 1 or (int)x == 0) ? CRGB::Black : getPixel(x - 1, y + 1));
        }
      }
      break;
  }
  ...
  for (byte i = 0; i < scale; i++) {
    ...
    if ((x1 >= 0 and x1 <= WIDTH-1) and (y1 >= 0 and y1 <= HEIGHT -1)) drawPixel(x1, y1, ColorFromPalette(pal[palette], beatsin88(256*12.*speedFactor + i*256, 0, 255), 255));
  }
