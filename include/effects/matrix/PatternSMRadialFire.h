...
public:
    PatternSMRadialFire() :
        EffectWithId<PatternSMRadialFire>("RadialFire")
    {
    }
    PatternSMRadialFire(const JsonObjectConst &jsonObject) :
        EffectWithId<PatternSMRadialFire>(jsonObject)
    {
    }
    ...
    void Draw() override
    {
        ...
        for (uint8_t x = 0; x < MATRIX_WIDTH; x++)
        {
            for (uint8_t y = 0; y < MATRIX_HEIGHT; y++)
            {
                ...
                uint8_t radius =
                    rMap[x][y].unscaled_radius; // Use the unscaled radius
                int16_t Bri =
                    inoise8(angle * scaleX, (radius * scaleY) - t) -
                    radius * (255 / MATRIX_HEIGHT);
                uint8_t Col = Bri;
                ...
                // If the palette is paused, we use it to color the fire,
                // otherwise we just use red Step 1: Calculate normalized
                // color value
                ...
                CRGB heatColor =
                    GetBlackBodyHeatColor(normalizedCol, baseColor);
