...
#endif
{
 private:
  void mydrawLine(byte x, byte x1, byte y, CHSV color, bool dot, bool grad,
                  byte numline, byte side, byte sinOff,
                  uint16_t a) {  // my ugly hori line draw function )))
    ...
    for (uint16_t i = 1; i <= steps; i++) {
      ...
      if (grad)
        g()->leds[index] %=
            (sin8(numline * 8 + side * 64 + a + sinOff) + i * 255 / steps) /
 ...
 public:
  ...
  virtual void Draw() override {
    ...
    for (uint16_t i = 0; i < MATRIX_HEIGHT; i++) {
      ...
      if (x2 < x3)
        mydrawLine(x2, x3, i, CHSV(hueColor + 64, 255, 255), 1, 1, i, 1, sinOff,
                   a);
      if (x3 < x4)
        mydrawLine(x3, x4, i, CHSV(hueColor + 128, 255, 255), 1, 1, i, 2,
                   sinOff, a);
      if (x4 < x1)
        mydrawLine(x4, x1, i, CHSV(hueColor + 192, 255, 255), 1, 1, i, 3,
                   sinOff, a);
    }
