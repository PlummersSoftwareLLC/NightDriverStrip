...
class PatternSMLightning : public LEDStripEffect
{
  ...
  public:
    PatternSMLightning() : LEDStripEffect(EFFECT_MATRIX_SMLIGHTNING, "Lightning")
    {
    }

    PatternSMLightning(const JsonObjectConst &jsonObject) : LEDStripEffect(jsonObject)
    {
    }
    ...
    void Draw() override
    {
        ...
        if (call == 0 || call == 20 || call == 50 && aux1 == 1)
        { // draw main line
            int y = matrixHeight;
        ...
        { // draw only parts of 'searching' lines
            ...
            {
                int y = matrixHeight;
                int16_t x0 = aux0 - RANDOM(i, y); // substract the first offset so that all
