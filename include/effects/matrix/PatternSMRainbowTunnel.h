...
#if ENABLE_AUDIO
class PatternSMRainbowTunnel : public BeatEffectBase, public LEDStripEffect
#else
...
public:
  ...
  for (uint8_t x = 0; x < LED_COLS; x++) {
    for (uint8_t y = 0; y < LED_ROWS; y++) {
      ...
      g()->leds[g()->xy(x, y)] = CHSV((angle*scaleX)-t+(radius*scaleY), 255, constrain(radius*2,0,255));
    }
  }
