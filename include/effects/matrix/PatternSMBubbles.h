...
#undef WU_WEIGHT
    ...
    void drawPixelXYF(float x, float y, CRGB color) //, uint8_t darklevel = 0U)
    {
        if ((x > MATRIX_WIDTH - 1) || (y > MATRIX_HEIGHT - 1))
            return;
        if ((x < 0) || (y < 0))
            return; //не похоже, чтобы отрицательные значения хоть
  ...
  public:
    PatternSMBubbles() : LEDStripEffect(EFFECT_MATRIX_SMBUBBLES, "Bubbles")
    {
    }

    PatternSMBubbles(const JsonObjectConst &jsonObject) : LEDStripEffect(jsonObject)
    {
    }
   ...
   if   (xboid->bbrightness >= 255) xboid-> bbrightness = 255;
            ...
            drawPixelXYF(boid->location.x, boid->location.y,
                         ColorFromPalette(OceanColors_p, boid->hue * 15, xboid->bbrightness, LINEARBLEND));

            if (boid->location.x < 0 || boid->location.x >= MATRIX_WIDTH || boid->location.y < 0 ||
