...
{
private:
    // Radius is the lesser of the height and width so that the round
    // clock can fit on rectangular display
    float radius;
...
public:
    ...
    PatternClock(const JsonObjectConst &jsonObject) :
        EffectWithId<PatternClock>(jsonObject)
    {
    }
    ...
    void Draw() override
    {
        ...
        // Draw the clock face, outer ring and inner dot where the hands
        // mount

        radius = std::min(MATRIX_WIDTH, MATRIX_HEIGHT) / 2 - 0.5;

        g()->Clear();
        g()->DrawSafeCircle(MATRIX_WIDTH / 2, MATRIX_HEIGHT / 2, 1,
                            CRGB::Blue);

        // Draw the hour ticks around the outside of the clock every 30
        // degrees
        ...
        for (int z = 0; z < 360; z = z + 30)
        {
            ...
            int x2 =
                (MATRIX_CENTER_X +
                 round(sin(angle) *
                       (radius -
                        4))); // Extra 0.5 helps rounding land more evenly
        ...
        g()->DrawSafeCircle(MATRIX_WIDTH / 2, MATRIX_HEIGHT / 2, radius,
                            CRGB::Blue);
        g()->DrawSafeCircle(MATRIX_WIDTH / 2, MATRIX_HEIGHT / 2,
                            radius + 1, CRGB::Green);
        ...
        g()->drawLine(MATRIX_CENTER_X, MATRIX_CENTER_Y, x3, y3,
                      CRGB::White);
        ...
        int x2 =
            (MATRIX_CENTER_X +
             round(sin(angle) *
                   (radius -
                    1))); // Extra 0.5 helps rounding land more evenly
