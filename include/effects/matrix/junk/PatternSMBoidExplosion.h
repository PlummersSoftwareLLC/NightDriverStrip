...
#if ENABLE_AUDIO
class PatternSMBoidExplosion : public BeatEffectBase, public LEDStripEffect
#else
...
#define NUM_PARTICLES 75  //set this to the number of particles. the varialbe describes what it's supposed to be. it works with 50 but it's a little slow. on an esp32 it looks pretty nice at that number 15 is a safe number
...
void ChangePalettePeriodically(){
    ...
    if( lastSecond != secondHand) {
        ...
        if( secondHand ==  0)  { currentPalette = HeatColors_p; currentBlending = NOBLEND;  }
        if( secondHand == 10)  { currentPalette = HeatColors_p; currentBlending = LINEARBLEND; }
        if( secondHand == 15)  { currentPalette = LavaColors_p; currentBlending = NOBLEND; }
        if( secondHand == 20)  { currentPalette = LavaColors_p; currentBlending = LINEARBLEND; }
        if( secondHand == 25)  { currentPalette = ForestColors_p; currentBlending = NOBLEND; }
        if( secondHand == 30)  { currentPalette = ForestColors_p; currentBlending = LINEARBLEND; }
        if( secondHand == 35)  { currentPalette = OceanColors_p; currentBlending = NOBLEND; }
        if( secondHand == 40)  { currentPalette = OceanColors_p; currentBlending = LINEARBLEND; }
        if( secondHand == 45)  { currentPalette = CloudColors_p; currentBlending = NOBLEND; }
        if( secondHand == 50)  { currentPalette = CloudColors_p; currentBlending = LINEARBLEND; }
    }
}
...
class Attractor {
  public:
    ...
    float coldiv;
  Attractor() {
    location = PVector(ROWS / (float)2.0, COLS / (float)coldiv);//PVector(ROWS / (float)random((float)1.1,(float)4.0), COLS / (float)random((float)1.1,(float)4.0));
