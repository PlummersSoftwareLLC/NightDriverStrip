...
#if ENABLE_AUDIO
class PatternSMSand : public BeatEffectBase,
...
#if ENABLE_AUDIO
  static constexpr uint8_t top_reserve =
      1;  // I'm not sure that every place that needs it has it.
#else
    ...
    for (uint8_t y = 0; y < HEIGHT - top_reserve; y++)  // Skip audio line
      for (uint8_t x = 0; x < WIDTH; x++)
        if (auto me = getPixColorXY(x, y))  // checking for every grain of sand
          if (!getPixColorXY(x,
                             y - 1)) {  // if it's empty below us, we just fall
            drawPixelXY(x, y - 1, me);
            drawPixelXY(x, y, CRGB::Black);
          } else if (x > 0U && !getPixColorXY(x - 1, y - 1) && x < WIDTH - 1 &&
                     !getPixColorXY(x + 1, y - 1)) {  // if we have a peak
            if (random8(2U))
            ...
            pcnt = y - 1;
                     ...
                     !getPixColorXY(x - 1, y - 1)) {  // a a slope to the left
            drawPixelXY(x - 1, y - 1, me);
            drawPixelXY(x, y, CRGB::Black);
            pcnt = y - 1;
          } else if (x < WIDTH - 1 &&
                     !getPixColorXY(
                         x + 1, y - 1)) {  // if below us slopes to the right
