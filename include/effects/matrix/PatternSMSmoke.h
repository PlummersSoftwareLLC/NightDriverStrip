...
public:
    PatternSMSmoke() : EffectWithId<PatternSMSmoke>("Smoke")
    {
    }
    ...
    void Draw() override
    {
        ...
        if (deltaHue &
            0x01) //((deltaHue >> 2U) == 0U) // (orig) I'd like to connect
                  //some kind of multiplier to the
                  // color change delay, but I don't know what...
        ...
        // deltaHue2--;
        if (random8(WIDTH) !=
            0U) // (orig) // the counter spiral does not always move
                // synchronously with the main one.
            deltaHue2--;
        ...
        {
            g()->leds[XY((deltaHue + y + 1U) % WIDTH, HEIGHT - 1U - y)] +=
                color;
            g()->leds[XY((deltaHue + y) % WIDTH, HEIGHT - 1U - y)] +=
                color2; // color2
            g()->leds[XY((deltaHue2 + y) % WIDTH, y)] += color;
            g()->leds[XY((deltaHue2 + y + 1U) % WIDTH, y)] +=
                color2; // color2
        }
        ...
        // Lower number for thicker, more static fog. Higher for more
        // wisp.
        g()->MoveFractionalNoiseX(3);
        g()->MoveFractionalNoiseY(3);
        // Without this, we get tornadoes where the diagonals cross as
        // there's an excess of set pixels there.
        g()->BlurFrame(10);
    }
};
