...
#define NUM_PARTICLES                                                                                                  \
    ...
    void drawPixelXYF(float x, float y, CRGB color)
    {
        ...
        static const uint8_t wu[4] = {WU_WEIGHT(ix, iy), WU_WEIGHT(xx, iy), WU_WEIGHT(ix, yy), WU_WEIGHT(xx, yy)};
  ...
  public:
    PatternSMBoidExplosion() : LEDStripEffect(EFFECT_MATRIX_SMBOID_EXPLOSION, "Boid Explosion")
    {
    }

    PatternSMBoidExplosion(const JsonObjectConst &jsonObject) : LEDStripEffect(jsonObject)
    {
    }
