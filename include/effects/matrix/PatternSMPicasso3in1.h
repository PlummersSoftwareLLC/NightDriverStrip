...
private:
    ...
    uint8_t Scale =
        2; // 1-100 is image type and count. THis should be a Setting 0-33
           // = P1, 34-68 = P2, 68-99 = Picasso3 P1 - Scale is number of
           // independent lines drawn w/ trailers. Safe=1-8 20 or so =
           // Pink Floyd 'Pulse' laser show. P2 - 34 & up. Scale -33 ==
           // number of vertices on a connected polyline "wire" rotating
           // in 3d. 38 up? SLOW! P3 - 68 & up. Scale -68 -2 == number of
           // circles  *68=2, 69=3, 70=4, etc. 80 up? SLOW
    static constexpr int trackingOBJECT_MAX_COUNT = 100U;
    // maximum number of tracked objects (greatly affects memory
    // consumption)
    ...
    uint8_t
        enlargedObjectNUM; // используемое в эффекте количество объектов
    ...
    void PicassoGenerate(bool reset)
    {
        ...
        if (reset || loadingFlag)
        {
            ...
            for (uint8_t i = 0; i < enlargedObjectNUM; i++)
            {
                ...
                trackingObjects[i].speedY =
                    +((-maxSpeed / 3) +
                      (maxSpeed * (float)random8(1, 100) / 100));
                trackingObjects[i].speedY +=
                    trackingObjects[i].speedY > 0 ? minSpeed : -minSpeed;

                trackingObjects[i].shift =
                    +((-maxSpeed / 2) +
                      (maxSpeed * (float)random8(1, 100) / 100));
                trackingObjects[i].shift +=
                    trackingObjects[i].shift > 0 ? minSpeed : -minSpeed;
        ...
        for (uint8_t i = 0; i < enlargedObjectNUM; i++)
        {
            if (reset)
            {
                ...
                trackingObjects[i].speedX =
                    (trackingObjects[i].state - trackingObjects[i].hue) /
                    25;
            }
            if (trackingObjects[i].state != trackingObjects[i].hue &&
                trackingObjects[i].speedX)
            {
    ...
    void PicassoPosition()
    {
        for (uint8_t i = 0; i < enlargedObjectNUM; i++)
        {
            if (trackingObjects[i].posX + trackingObjects[i].speedY >
                    MATRIX_WIDTH ||
            ...
            if (trackingObjects[i].posY + trackingObjects[i].shift >
                    MATRIX_HEIGHT ||
    ...
    void PicassoRoutine1()
    {
        ...
        {
            g()->drawLine(trackingObjects[i].posX,
                          trackingObjects[i].posY,
                          trackingObjects[i + 1U].posX,
                          trackingObjects[i + 1U].posY,
                          CHSV(trackingObjects[i].hue, 255U, 255U));
    ...
    void PicassoRoutine3()
    {
        ...
        for (uint8_t i = 0; i < enlargedObjectNUM - 2U; i += 2)
            g()->DrawSafeCircle(
                fabs(trackingObjects[i].posX -
                     trackingObjects[i + 1U].posX),
                fabs(trackingObjects[i].posY -
                     trackingObjects[i + 1U].posX),
                fabs(trackingObjects[i].posX - trackingObjects[i].posY),
                CHSV(trackingObjects[i].hue, 255U, 255U));
...
public:
    PatternSMPicasso3in1() :
        EffectWithId<PatternSMPicasso3in1>("Picasso"), _scale(-1)
    {
    }

    PatternSMPicasso3in1(const String &name, int scale) :
        EffectWithId<PatternSMPicasso3in1>(name), _scale(scale)
    {
    }
    ...
    void RecalibrateDrawnObjects()
    {
        if (Scale < 34U) // если масштаб до 34
            enlargedObjectNUM =
                (Scale - 1U) / 32.0 * (enlargedOBJECT_MAX_COUNT - 3U) +
                3U;
        else if (Scale >= 68U) // если масштаб больше 67
            enlargedObjectNUM =
                (Scale - 68U) / 32.0 * (enlargedOBJECT_MAX_COUNT - 3U) +
                3U;
        else // для масштабов посередине
            enlargedObjectNUM =
                (Scale - 34U) / 33.0 * (enlargedOBJECT_MAX_COUNT - 1U) +
                1U;
    }
