...
#if ENABLE_AUDIO
class PatternSMSinusoidSin16 : public BeatEffectBase,
...
#endif
{
  ...
  */
  ...
  void Draw() override {
    ...
    uint16_t _scale = (((Scale - 1U) % 9U) * 10U + 80U)
                      << 7U;  // = fmap(scale, 1, 255, 0.1, 3);
    float _scale2 =
        (float)((Scale - 1U) % 9U) * 0.2 + 0.4;  // для спиралей на sinf
    uint16_t _scale3 =
        ((Scale - 1U) % 9U) * 1638U + 3276U;  // для спиралей на sin16

    CRGB color;

    float center1x =
        float(e_s3_size * cos16(speedfactor * 72.0874 * time_shift)) / 0x7FFF -
        emitterX;
    float center1y =
        float(e_s3_size * sin16(speedfactor * 98.301 * time_shift)) / 0x7FFF -
        emitterY;
    float center2x =
        float(e_s3_size * sin16(speedfactor * 68.8107 * time_shift)) / 0x7FFF -
        emitterX;
    float center2y =
        float(e_s3_size * cos16(speedfactor * 65.534 * time_shift)) / 0x7FFF -
        emitterY;
    float center3x =
        float(e_s3_size * sin16(speedfactor * 134.3447 * time_shift)) / 0x7FFF -
        emitterX;
    // float center3x3x = float(e_s3_size * sin16(float(speedfactor * 134.3447 *
    // time_shift))) / 0x7FFF - xemitterX;
    float center3y =
        float(e_s3_size * cos16(speedfactor * 170.3884 * time_shift)) / 0x7FFF -
        emitterY;
      ...
      case 0:  // Sinusoid I
        for (uint8_t y = 0; y < MATRIX_HEIGHT; y++) {
          for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
            ...
            int8_t v =
                127 *
                (1 + float(sin16(_scale * SQRT_VARIANT(cx * cx + cy * cy))) /
                         0x7FFF);
            color.r = v;
            cx = x + center3x;
            cy = y + center3y;
      ...
      case 4:  // changed by stepko //colored sinusoid
        for (uint8_t y = 0; y < MATRIX_HEIGHT; y++) {
          for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
            ...
            int8_t v =
                127 *
                (1 + float(sin16(_scale * (beatsin16(2, 1000, 1750) / 2550.) *
            ...
            v = 127 *
                (1 + float(sin16(_scale * (beatsin16(1, 570, 1050) / 2250.) *
                                     SQRT_VARIANT(((cx * cx) + (cy * cy))) +
            ...
            // time_shift * speedfactor));
            v = 127 *
                (1 + float(cos16(_scale * (beatsin16(3, 1900, 2550) / 2550.) *
                                     SQRT_VARIANT(((cx * cx) + (cy * cy))) +
      ...
      case 5:  // changed by stepko //sinusoid in net
        for (uint8_t y = 0; y < MATRIX_HEIGHT; y++) {
          for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
            ...
            v = 127 *
                (1 +
                 float(sin16(_scale * (x + 0.005 * time_shift * speedfactor))) /
            ...
            v = 127 * (1 + float(sin16(_scale * (y + 0.0055 * time_shift *
                                                         speedfactor))) /
                               0x7FFF);  // proper by SottNick
            color.r = ~v;
            drawPixelXY(x, y, color);
      ...
      case 6:  // changed by stepko //spiral
        for (uint8_t y = 0; y < MATRIX_HEIGHT; y++) {
          for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
            ...
            uint8_t v =
                127 *
                (1 + sinf(3 * atan2(cy, cx) +
                          _scale2 * hypot(cy, cx)));  // proper by SottNick
            ...
            // proper by SottNick
            v = 127 * (1 + float(sin16(atan2(cy, cx) * 31255 +
                                       _scale3 * hypot(cy, cx))) /
                               0x7FFF);  // proper by SottNick
            color.g = v;
            drawPixelXY(x, y, color);
