...
// coordinate system had 0,0 in the LL corner. We have 0,0 in UL. BUGBUG:
// This would look better if the snowflakes took longer to decay. I can't
// find the magic for blur2d().

#if ENABLE_AUDIO
class PatternSMStrobeDiffusion :
    public BeatEffectBase,
    public EffectWithId<PatternSMStrobeDiffusion>
#else
class PatternSMStrobeDiffusion :
    public EffectWithId<PatternSMStrobeDiffusion>
#endif
...
private:
    ...
    std::bitset<MATRIX_WIDTH * MATRIX_HEIGHT>
        noise3d[MATRIX_WIDTH * MATRIX_HEIGHT]; // Locations of snowflakes.
    ...
    const int LOW_DELAY =
        0; // This is used to set FPSdelay ... which is never used.
...
public:
    ...
    void VirtualSnow()
    {
        ...
        {
            // Don't copy the very top (usable) line that we're about to
            // fill with fresh snowflakes.
            for (uint8_t y = MATRIX_HEIGHT - 1; y > top_line_offset; y--)
            {
                ...
                {
                    g()->drawPixel(x, y,
                                   CHSV(170, 5U, 127 + random8(128)));
                }
            }
        }

        // Scroll the flakes above more often than we add new flakes
        // below.
        //		  if ((ct & 0x07) != 0) return;
    ...
    void Draw() override
    {
...
#if ENABLE_AUDIO
        ProcessAudio(); // Without this, HandleBeat has no audio to
                        // process.
#endif
        ...
        if (Scale > 50)
        {
            ...
            g()->blur2d(g()->leds, MATRIX_WIDTH, 0, MATRIX_HEIGHT,
                        top_line_offset, beatsin8(3, 64, 80));
            // g()->blur2d(g()->leds, MATRIX_WIDTH, 0, MATRIX_HEIGHT,
            // top_line_offset, 24);
            FPSdelay = LOW_DELAY;
            STEP     = 1U;
        ...
        // Tuned by hand. Too slow and they just flash and show up in new
        // locations.
        EVERY_N_MILLIS(30)
        {
            VirtualSnow();
        ...
        // Much of this could be calls to lineDraw() but maybe being able
        // to do both ends while iterating through the loop is a win.
        ...
        for (uint8_t y = 0; y < rows; y++)
        {
            if (dir)
            {
                if ((step % STEP) == 0)
                { // small layers
                    g()->drawPixel(MATRIX_WIDTH - 1, y * 3 + DELTA,
                                   CHSV(step, 255U, 255U));
                }
                else
                {
            ...
            else
            {
                if ((step % STEP) == 0)
                { // big layers
                    g()->drawPixel(0, y * 3 + DELTA,
                                   CHSV(step + deltaHue, 255U, 255U));
                }
                else
                {
            ...
            for (uint8_t x = 1U; x < MATRIX_WIDTH; x++)
            {
                if (dir)
                { // <==
                    g()->drawPixel(x, y * 3 + DELTA,
                                   g()->getPixel(x, y * 3 + DELTA));
                }
                else
                { // ==>
                    g()->drawPixel(
                        MATRIX_WIDTH - x, y * 3 + DELTA,
                        g()->getPixel(MATRIX_WIDTH - x, y * 3 + DELTA));
                }
            }
            dir = !dir;
        }
