...
#if ENABLE_AUDIO
class PatternSMFire2021 : public BeatEffectBase, public LEDStripEffect
#else
...
public:
  ...
  void Start() override
  {
    ...
    step = map(Speed*Speed, 1U, 65025U, (deltaValue-1U)/2U+1U, deltaValue * 18U + 44); // корректируем скорость эффекта в наш диапазон допустимых
    //deltaValue = (((Scale - 1U) % 11U + 2U) << 4U); // ширина языков пламени (масштаб шума Перлина)
  ...
  for (byte x = 0; x < MATRIX_WIDTH; x++) {
    for (byte y = 0; y < MATRIX_HEIGHT; y++) {
      ...
      nblend(g()->leds[g()->xy(x, MATRIX_HEIGHT-1-y)], ColorFromPalette(*curPalette, Col, Bri), pcnt);
    }
  }
