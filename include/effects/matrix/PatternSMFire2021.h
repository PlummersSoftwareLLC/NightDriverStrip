...
private:
    ...
    uint8_t step; // какой-нибудь счётчик кадров или последовательностей
                  // операций
...
public:
    ...
    PatternSMFire2021(const JsonObjectConst &jsonObject) :
        EffectWithId<PatternSMFire2021>(jsonObject)
    {
    }
    ...
    void Start() override
    {
        g()->Clear();
        if (Scale > 100U)
            Scale = 100U; // чтобы не было проблем при прошивке без
                          // очистки памяти
        deltaValue =
            Scale * 0.0899; // /100.0F * ((sizeof(palette_arr)
                            // /sizeof(TProgmemRGBPalette16 *))-0.01F));
        deltaValue = ((Scale - 1U) % 11U + 1U);
        step =
            ::map(Speed * Speed, 1U, 65025U, (deltaValue - 1U) / 2U + 1U,
                  deltaValue * 18U + 44); // корректируем скорость эффекта
                                          // в наш диапазон допустимых
        // deltaValue = (((Scale - 1U) % 11U + 2U) << 4U); // ширина
        // языков пламени (масштаб шума Перлина)
    ...
    void Draw() override
    {
        ...
        for (unsigned x = 0; x < MATRIX_WIDTH; x++)
        {
            ...
            {
                int16_t Bri = inoise8(x * deltaValue,
                                      (y * deltaValue) - ff_x, ff_z) -
                              (y * (255 / MATRIX_HEIGHT));
                uint8_t Col =
                    Bri; // inoise8(x * deltaValue, (y * deltaValue) -
                         // ff_x, ff_z) - (y * (255 / MATRIX_HEIGHT));
                if (Bri < 0)
                    Bri = 0;
                if (Bri != 0)
                    Bri = 256 - (Bri * 0.2);

                // Get the flame color using the black body radiation
                // approximation, but when the palette is paused we make
                // flame in that base color instead of the normal red
