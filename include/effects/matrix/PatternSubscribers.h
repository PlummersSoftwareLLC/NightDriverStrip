...
private:
    ...
    void SubscriberReader()
    {
        ...
        if (guidUpdated || !msLastCheck || (!succeededBefore && msSinceLastCheck > SUB_CHECK_ERROR_INTERVAL) ||
    ...
    void UpdateSubscribers()
    {
        ...
        {
            debugW("Error fetching subscribers for channel %s (GUID %s)", youtubeChannelName.c_str(),
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (commaIndex < 0)
            {
                debugW("Malformed response while fetching subscribers for channel %s (GUID %s)",
        ...
        debugI("Got YouTube subscriber count for channel %s (GUID %s)", youtubeChannelName.c_str(),
               youtubeChannelGuid.c_str());
...
protected:
    ...
    EffectSettingSpecs *FillSettingSpecs() override
    {
        ...
        {
            mySettingSpecs.emplace_back(NAME_OF(youtubeChannelGuid), "YouTube channel GUID",
                                        "The <a href=\"http://tools.tastethecode.com/youtube-sight\">YouTube Sight</a> "
                                        "GUID of the channel for which "
                                        "the effect should show subscriber information.",
                                        SettingSpec::SettingType::String);
            mySettingSpecs.emplace_back(NAME_OF(backgroundColor), "Background Color", "Color for the background",
                                        SettingSpec::SettingType::Color);
            mySettingSpecs.emplace_back(NAME_OF(borderColor), "Border Color", "Color for the border around the edge",
                                        SettingSpec::SettingType::Color);
            mySettingSpecs
...
public:
    ...
    bool Init(std::vector<std::shared_ptr<GFXBase>> &gfx) override
    {
        ...
        readerIndex =
            g_ptrSystem->NetworkReader().RegisterReader([this] { SubscriberReader(); }, SUB_READER_INTERVAL, true);
