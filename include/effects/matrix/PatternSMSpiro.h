...
class PatternSMSpiro : public LEDStripEffect
{
  ...
  public:
    PatternSMSpiro() : LEDStripEffect(EFFECT_MATRIX_SMSPIRO, "Spiro")
    {
    }

    PatternSMSpiro(const JsonObjectConst &jsonObject) : LEDStripEffect(jsonObject)
    {
    }
    ...
    void Draw() override
    {
        ...
        for (uint8_t i = 0; i < spirocount; i++)
        {
            ...
            if (x2 < WIDTH && y2 < HEIGHT) // добавил проверки. не знаю, почему
                ...
                g()->leds[XY(x2, y2)] += (CRGB)ColorFromPalette(*curPalette, hue + i * spirooffset);
