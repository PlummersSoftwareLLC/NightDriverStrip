...
#if ENABLE_AUDIO
class PatternSMWisp : public BeatEffectBase, public LEDStripEffect
#else
...
void MoveFractionalNoiseX(int8_t amplitude = 1, float shift = 0) {
  for (uint8_t y = 0; y < MATRIX_HEIGHT; y++) {
    ...
    for (uint8_t x = 0 ; x < MATRIX_WIDTH; x++) {
      ...
      if ((zD >= 0) && (zD < MATRIX_WIDTH)) PixelA = g()->leds[g()->xy(zD, y)];
      CRGB PixelB = CRGB::Black ;
      if ((zF >= 0) && (zF < MATRIX_WIDTH)) PixelB = g()->leds[g()->xy(zF, y)];
      ledsbuff[g()->xy(x, y)] = (PixelA.nscale8(ease8InOutApprox(255 - fraction))) + (PixelB.nscale8(ease8InOutApprox(fraction)));   // lerp8by8(PixelA, PixelB, fraction );
    }
  }
...
void MoveFractionalNoiseY(int8_t amplitude = 1, float shift = 0) {
  for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
    ...
    for (uint8_t y = 0 ; y < MATRIX_HEIGHT; y++) {
      ...
      CRGB PixelA = CRGB::Black ;
      // NightDriver change: Keep it out of the AUDIO line or you'll get green ghost wisps (which are kinda cool)
      if ((zD >= 1) && (zD < MATRIX_HEIGHT)) PixelA = g()->leds[g()->xy(x, zD)];
      CRGB PixelB = CRGB::Black ;
      if ((zF >= 0) && (zF < MATRIX_HEIGHT)) PixelB = g()->leds[g()->xy(x, zF)];
      ledsbuff[g()->xy(x, y)] = (PixelA.nscale8(ease8InOutApprox(255 - fraction))) + (PixelB.nscale8(ease8InOutApprox(fraction)));
    }
  }
  memcpy(g()->leds, ledsbuff, sizeof(CRGB)* NUM_LEDS);
}
