...
#if ENABLE_AUDIO
class PatternSMGoogleNexus : public BeatEffectBase, public LEDStripEffect
#else
...
{
  if (x<0 || y<0 || x>((float)MATRIX_WIDTH) || y>((float)MATRIX_HEIGHT)) return;
...
public:
  ...
  virtual void Start() override
  {
    ...
    for (byte i = 0; i < GOOGLE_NEXUS; i++) {
      ...
      dotColor[i] = ColorFromPalette(RainbowColors_p, random8(0, 9) * 31, 255); // dot color
 ...
 fadeAllChannelsToBlackBy(map(speed, 1, 255, 11, 33));
  ...
  for (byte i = 0; i < map(scale, 1, 255, 4, GOOGLE_NEXUS); i++) {
    ...
    // // Make it seamless in Y. And move the blob to the beginning of the track
    if (dotPosY[i] < 0) {
