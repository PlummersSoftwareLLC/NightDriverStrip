...
#define GOOGLE_NEXUS (MATRIX_WIDTH)
  ...
  double fmap(const double x, const double in_min, const double in_max,
  ...
  void resetDot(uint8_t idx) {
    ...
    dotColor[idx] =
        ColorFromPalette(RainbowColors_p, random(0, 9) * 31, 255);  // color
    dotAccel[idx] = (float)random(5, 10) /
                    70;  // make particles slightly different acceleration
    switch (dotDirect[idx]) {
      case 0:  // вверх
        dotPosX[idx] =
            random8(0, MATRIX_WIDTH);  // Scatter drops across the width
  ...
  void Start() override {
    ...
    for (byte i = 0; i < GOOGLE_NEXUS; i++) {
      ...
      dotColor[i] = ColorFromPalette(RainbowColors_p, random8(0, 9) * 31,
                                     255);  // dot color
      dotAccel[i] = (float)random(5, 11) /
                    70;  // make particles each slightly different speed
    }
  }
