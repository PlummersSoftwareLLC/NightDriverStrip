...
#if ENABLE_AUDIO
class PatternSMBlurringColors : public BeatEffectBase, public LEDStripEffect
#else
...
bool    trackingObjectIsShift[trackingOBJECT_MAX_COUNT];  // BugBug: cold be a Std::Biefield<T>
...
public:
  ...
  void Start() override
  {
    ...
    for(int j = 0; j<enlargedObjectNUM; j++){
      ...
      boids[j].velocity.y = 1;//sqrt(0.0626-boids[j].velocity.x*boids[j].velocity.x) /  8.; // скорость источников в восемь раз ниже, чем скорость частиц
