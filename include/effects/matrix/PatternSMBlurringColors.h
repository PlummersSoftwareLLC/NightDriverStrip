...
#undef WU_WEIGHT
  ...
  void particlesUpdate2(uint8_t i) {
    ...
    if (trackingObjectState[i] == 0 || trackingObjectPosX[i] <= -1 ||
        trackingObjectPosX[i] >= WIDTH || trackingObjectPosY[i] <= -1 ||
  ...
  void fountainsEmit(uint8_t i) {
    ...
    trackingObjectSpeedX[i] = ((float)random8() - 127.) /
                              512.;  // random(_hVar)-_constVel; // particle->vx
    trackingObjectSpeedY[i] = sqrt(
    ...
    trackingObjectState[i] =
        random8(50, 250);  // random8(minLife, maxLife);// particle->ttl
    if (Speed & 0x01)
 ...
 public:
  ...
  void Start() override {
    ...
    deltaValue =
        trackingOBJECT_MAX_COUNT / (sqrt(CENTER_X_MAJOR * CENTER_X_MAJOR +
    ...
    for (int j = 0; j < enlargedObjectNUM; j++) {
      ...
      boids[j].velocity.y =
          1;  // sqrt(0.0626-boids[j].velocity.x*boids[j].velocity.x) /  8.; //
