...
//
// History:     May-30-2023         Davepl      Created for
// NightDriverStrip
//
//---------------------------------------------------------------------------
...
// Be careful of structure packing rules when adding elements to this
// structure The client will be expecting the data to be packed in a
// certain way (tight). If you add a field that is not a multiple of 4
// bytes, you may need to add padding to keep the structure packed the way
...
// LEDViewer
//
// LEDViewer is a class that listens on a TCP port for a connection a
// client and responds with the current state of the LED array.  This
// allows a client to monitor the state of the LED array in real time.
...
public:
    explicit LEDViewer(int port) : _port(port), _server_fd(-1)
    {
    ...
    bool SendPacket(int socket, void *pData, size_t cbSize)
    {
        ...
        debugV("Sending Packet:  %02X, %02X, %02X, %02X, %02X, %02X, "
