...
// File:        hashing.h
//
// NightDriverStrip - (c) 2025 Plummer's Software LLC.  All Rights
// Reserved.
//
...
template <typename H> struct traits;
...
// Generic FNV-1a over raw bytes for any supported hash type H
// (uint32_t/uint64_t)
template <typename H>
constexpr H hash_bytes(const void *data, size_t len,
                       H seed = traits<H>::offset)
{
    H h           = seed;
...
// Supports arithmetic types, enums, string_view, C-strings, and
...
constexpr H hash(const T &v, H h = traits<H>::offset)
{
    ...
    else
    {
        static_assert(dependent_false<T>::value,
                      "Unsupported argument type for hashing");
    }
}
...
// Hashes a std::vector by hashing its size and then each element in
// order.
template <typename H, typename T>
...
template <typename H, typename T> inline void hash_append(H &h, T &&v)
{
    h = hash<H>(std::forward<T>(v), h);
}

// Hashes a pack of arguments in order, starting with an initial hash
// value.
...
// Appends a pack of arguments to an existing hash, updating the hash in
// place.
template <typename H, typename... As>
...
template <typename H> constexpr size_t hash_string_length()
{
    return sizeof(H) * 2;
}

// Converts a hash value to a hexadecimal Arduino String of fixed length.
template <typename H> inline String hash_to_string(H h)
{
