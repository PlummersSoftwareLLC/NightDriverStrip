...
var groupTargetsByDirectoryTree = (targets) => {
  const targetsWithOrder = targets.map((target, order) => ({ ...target, order })).sort(
    (a, b) => a.resolvedDest === b.resolvedDest ? 0 : a.resolvedDest > b.resolvedDest ? 1 : -1
  );
  const groups = {};
  for (const target of targetsWithOrder) {
    const { resolvedDest } = target;
    const parent = Object.keys(groups).find(
      (key) => isSubdirectoryOrEqual(key, resolvedDest)
    );
    if (parent) {
      groups[parent].push(target);
      continue;
    }
    const child = Object.keys(groups).find(
      (key) => isSubdirectoryOrEqual(resolvedDest, key)
    );
    if (child) {
