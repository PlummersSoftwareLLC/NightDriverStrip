...
// Description:
//
//   Provides definitions for GFXBase member functions that can't be
//   included in gfxbase.h because C++ won't allow us.
//
...
#if USE_NOISE
// The following functions are specializations of noise-related member
// function templates declared in gfxbase.h.
...
template <> void GFXBase::FillGetNoise<NoiseApproach::One>()
{
    ...
    {
        uint32_t ioffset =
            _ptrNoise->noise_scale_x * (i - ((_height + 1) / 2));

        for (uint16_t j = 0; j < _height; j++)
        {
            uint32_t joffset =
                _ptrNoise->noise_scale_y * (j - ((_height + 1) / 2));
            uint16_t data = inoise16(_ptrNoise->noise_x + ioffset,
                                     _ptrNoise->noise_y + joffset,
                                     _ptrNoise->noise_z) >>
                            8;
...
template <> void GFXBase::FillGetNoise<NoiseApproach::Two>()
{
    for (uint8_t i = 0; i < MATRIX_WIDTH; i++)
    {
        ...
        {
            int32_t joffset =
                _ptrNoise->noise_scale_y * (j - CENTER_Y_MINOR);
            int8_t data = inoise16(_ptrNoise->noise_x + ioffset,
                                   _ptrNoise->noise_y + joffset,
                                   _ptrNoise->noise_z) >>
                          8;
...
template <>
void GFXBase::MoveFractionalNoiseX<NoiseApproach::One>(uint8_t amt,
                                                       uint8_t shift)
{
    std::unique_ptr<CRGB[]> ledsTemp =
        make_unique_psram<CRGB[]>(NUM_LEDS);
...
template <>
void GFXBase::MoveFractionalNoiseX<NoiseApproach::Two>(uint8_t amt,
                                                       uint8_t shift)
{
    for (uint8_t y = 0; y < MATRIX_HEIGHT; y++)
    {
        int16_t amount =
            ((int16_t)_ptrNoise->noise[0][y] - 128) * 2 * amt +
            shift * 256;
...
template <>
void GFXBase::MoveFractionalNoiseY<NoiseApproach::One>(uint8_t amt,
                                                       uint8_t shift)
{
    std::unique_ptr<CRGB[]> ledsTemp =
        make_unique_psram<CRGB[]>(NUM_LEDS);
...
template <>
void GFXBase::MoveFractionalNoiseY<NoiseApproach::Two>(uint8_t amt,
                                                       uint8_t shift)
{
    for (uint8_t x = 0; x < MATRIX_WIDTH; x++)
    {
        int16_t amount =
            ((int16_t)_ptrNoise->noise[x][0] - 128) * 2 * amt +
            shift * 256;
        int8_t delta    = abs(amount) >> 8;
        int8_t fraction = abs(amount) & 255;
        for (uint8_t y = 0; y < MATRIX_HEIGHT; y++)
...
// This can't be in gfxbase.h because it uses the FillGetNoise() function
// template.

GFXBase::GFXBase(int w, int h) :
    Adafruit_GFX(w, h), _width(w), _height(h), _ledcount(w * h)
{
// Allocate boids for matrix effects (like PatternBounce) when we have
// matrix dimensions
#if MATRIX_HEIGHT > 1
    ...
    _ptrNoise =
        std::make_unique<Noise>(); // Avoid specific PSRAM allocation
                                   // since highly random access
    assert(_ptrNoise);
...
// Remove the XY macro definition that was set in gfxbase.h. In this file
// we won't use it beyond this point anyway.
#undef XY

// Dirty hack to support FastLED, which calls out of band to get the pixel
// index for "the" array, without any indication of which array or who's
// asking, so we assume the first matrix. If you have trouble with more
// than one matrix and some FastLED functions like blur2d, this would be
// why.
