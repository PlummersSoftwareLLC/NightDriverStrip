...
//    displays info like the IP address buffer depth and clock info, FPS,
//    etc.
//
...
DRAM_ATTR std::mutex
    Screen::_screenMutex; // The storage for the mutex of the screen class

// What page of screen we are showing
DRAM_ATTR uint8_t g_iCurrentPage =
    0; // Will initialize to last active page on first draw
...
// No border will be drawn so no inset margin needed on Heltec OLED
        ...
        display.setTextColor(display.GetTextColor(),
                             display.GetBkgndColor());
        display.setCursor(xMargin, yMargin);
        display.println(str_sprintf("%s:%dx%d %c %dK", FLASH_VERSION_NAME,
                                    g_ptrSystem->Devices().size(),
                                    NUM_LEDS, chStatus,
        ...
        else
        {
            ...
            display.println(str_sprintf(
                "%ddB:%d.%d.%d.%d", (int)labs(WiFi.RSSI()), address[0],
        ...
        display.println(
            str_sprintf("BUFR:%02d/%02d %dfps ", bufferManager.Depth(),
        ...
        if (display.height() >= lineHeight * 7)
        {
            ...
            display.println(
                str_sprintf("CPU: %3.0f%%, %3.0f%%  ",
                            taskManager.GetCPUUsagePercent(0),
                            taskManager.GetCPUUsagePercent(1)));
        }
        ...
        if (display.height() >= lineHeight * 8)
        {
            ...
            float ratio = (float)bufferManager.Depth() /
                          (float)bufferManager.BufferCount();
            ...
            {
                display.fillRect(xMargin + 1, top, filled, height,
                                 display.GetTextColor());
                display.fillRect(xMargin + filled, top, width - filled,
                                 height, display.GetBkgndColor());
                display.drawRect(xMargin, top, width, height,
                                 display.GetBorderColor());
            }
            else
            {
                display.fillRect(xMargin + 1, top + 1, filled, height - 2,
                                 color);
                display.fillRect(xMargin + filled, top + 1,
                                 width - filled, height - 2, bkgndColor);
                display.drawRect(xMargin, top, width, height,
                                 display.GetBorderColor());
            }
        }
...
{
protected:
    // Cached content top calculated using the header text size to avoid
    // shifting
    int _contentTop = 0;
    int ContentTop(Screen &display) const
    {
        return _contentTop > 0 ? _contentTop
                               : (display.fontHeight() * 3 + 4);
    }
...
public:
    ...
    void Draw(Screen &display, bool bRedraw) override
    {
        ...
        const uint16_t backColor = display.IsMonochrome()
                                       ? BLACK16
                                       : Screen::to16bit(CRGB(0, 0, 64));
        static int lasteffect =
            g_ptrSystem->EffectManager().GetCurrentEffectIndex();
        ...
        // Compute and cache content top using the header text size so it
        // stays stable
        ...
        if (bRedraw || lastFullDraw == 0 ||
            millis() - lastFullDraw > 1000)
        {
            lastFullDraw = millis();
            const int currentEffect =
                g_ptrSystem->EffectManager().GetCurrentEffectIndex();
            ...
            if (bRedraw || lasteffect != currentEffect ||
                sip != currentIP)
            {
                display.fillRect(0, 0, display.width(), topMargin,
                                 backColor);
                display.fillRect(
                    0, display.height() - display.BottomMargin,
                    display.width(), display.BottomMargin, backColor);
                display.fillRect(0, topMargin - 1, display.width(), 1,
                                 BLUE16);
                display.fillRect(
                    0, display.height() - display.BottomMargin + 1,
                    display.width(), 1, BLUE16);
                ...
                String sEffect =
                    String("Effect: ") + String(currentEffect + 1) +
                    String("/") +
                ...
                w = display.textWidth(
                    g_ptrSystem->EffectManager().GetCurrentEffectName());
                display.setCursor(display.width() / 2 - w / 2, yh);
                display.print(
                    g_ptrSystem->EffectManager().GetCurrentEffectName());
                yh += display.fontHeight();
                ...
                String sIP =
                    WiFi.isConnected() ? currentIP.c_str() : "No Wifi";
                display.setTextColor(display.GetBorderColor(), backColor);
            ...
            // Footer line
            String footer = str_sprintf(" LED: %2d  Scr: %02d",
                                        g_Values.FPS, (int)screenFPS);
#if ENABLE_AUDIO
            footer = str_sprintf(" LED: %2d  Aud: %2d Ser:%2d Scr: %02d",
                                 g_Values.FPS, g_Analyzer.AudioFPS(),
                                 g_Analyzer.SerialFPS(), (int)screenFPS);
#endif
            ...
            if (footer != lastFooter)
            {
                lastFooter = footer;
                display.fillRect(0,
                                 display.height() - display.BottomMargin,
                                 display.width(), 1, BLUE16);
                display.setTextColor(display.GetBorderColor(), backColor);
                display.setTextSize(1);
...
// Add spectrum analyzer content
#if ENABLE_AUDIO
        {
            ...
            const float ySizeVU =
                display.height() / 16; // height of each block
            const int cPixels = 16;
            const float xSize = xHalf / (float)cPixels + 1;
            const int litBlocks =
                (g_Analyzer.VURatioFade() / 2.0f) * cPixels;
            for (int iPixel = 0; iPixel < cPixels; iPixel++)
            {
                uint16_t color16 = iPixel > litBlocks
                                       ? BLACK16
                                       : display.to16bit(ColorFromPalette(
                                             vuPaletteGreen,
                                             iPixel * (256 / (cPixels))));
                display.fillRect(xHalf - iPixel * xSize, topMargin,
                                 xSize - 1, ySizeVU, color16);
                display.fillRect(xHalf + iPixel * xSize, topMargin,
                                 xSize - 1, ySizeVU, color16);
            }
            ...
            {
                CRGB bandColor = ColorFromPalette(
                    RainbowColors_p,
                    ((int)map(iBand, 0, NUM_BANDS, 0, 255)) % 256);
                int bandWidth = display.width() / NUM_BANDS;
                auto color16  = display.to16bit(bandColor);
                auto topSection =
                    bandHeight -
                    bandHeight * g_Analyzer.Peak2Decay(iBand);
                if (topSection > 0)
                    display.fillRect(iBand * bandWidth, spectrumTop,
                                     bandWidth - 1, topSection, BLACK16);
                auto val = min(1.0f, g_Analyzer.Peak2Decay(iBand));
                assert(bandHeight * val <= bandHeight);
                display.fillRect(iBand * bandWidth,
                                 spectrumTop + topSection, bandWidth - 1,
                                 bandHeight - topSection, color16);
            }
        }
#endif // ENABLE_AUDIO
    }
...
public:
    ...
    void Draw(Screen &display, bool bRedraw) override
    {
        ...
        const int contentHeight =
            display.height() - display.BottomMargin - contentTop;
        ...
        if (scale <= 0)
            scale =
                1; // No fractional downscale; ensure at least 1px per LED
        ...
        // Optional: clear content area on full redraw to avoid ghosting
        // around edges
        if (bRedraw)
            display.fillRect(0, contentTop, contentWidth, contentHeight,
                             BLACK16);
        ...
        for (int y = 0; y < mh; ++y)
        {
            for (int x = 0; x < mw; ++x)
            {
                ...
                // Draw filled rect; subtract 1 to create a fine 1px grid
                // line when scale > 1
                if (scale > 1)
...
// Screen::Update
// Draws the OLED/LCD screen with the current stats on connection, buffer,
// drawing, etc.
void IRAM_ATTR Screen::Update(bool bRedraw)
{
    ...
    if (!s_initialized)
    {
        ...
        // Use the configured default page, but ensure it's within valid
        // range
        g_iCurrentPage =
            std::clamp(DEFAULT_INFO_PAGE, 0, activeCount - 1);
#else
...
// Displays statistics on the Heltec's built in OLED board.  If you are
// using a different board, you would simply get rid of this or modify it
// to fit a screen you do have.  You could also try serial output, as it's
// on a low-pri thread it shouldn't disturb the primary cores.
...
void IRAM_ATTR Screen::RunUpdateLoop()
{
    ...
    bool bRedraw = true;
    // Lazy init of buttons when loop starts (after hardware/defines are
    // known)
    ...
    constexpr uint32_t kTargetFrameTimeMs =
        1000 / kTargetFPS; // 33.33ms for 30fps
    ...
    for (;;)
    {
        ...
        // bRedraw is set when the page changes so that it can get a full
        // redraw.  It is also set initially as nothing has been drawn for
        // any page yet
        ...
        {
            auto &pages = Pages();
            pages[g_iCurrentPage]->OnButtonPress(
                0); // BtnA -> button 0 (flip page by default)
            bRedraw = true;
        }
        if (M5.BtnB.wasPressed())
        {
            auto &pages = Pages();
            pages[g_iCurrentPage]->OnButtonPress(
                1); // BtnB -> button 1 (next effect by default)
            bRedraw = true;
        }
#endif
...
// Thin wrapper to preserve the existing FreeRTOS task entry point name
// while delegating to the new Screen::RunUpdateLoop() method.
