...
#include <algorithm>
    ...
    {
        if(buttonIndex == 0)
            g_ptrSystem->EffectManager().NextEffect(); // Default: advance to the next effect
        else if(buttonIndex == 1)
            Screen::FlipToNextPage(); // Default: flip to the next page
    }
      ...
      public:
        ...
        void Draw(Screen&display, bool bRedraw) override
        {
            ...
            else
            {
                ...
                display.println(str_sprintf("%ddB:%d.%d.%d.%d", (int) labs(WiFi.RSSI()), address[0], address[1], address[2],
                        address[3]));
            }
            ...
            display.println(
                    str_sprintf("BUFR:%02d/%02d %dfps ", bufferManager.Depth(), bufferManager.BufferCount(), g_Values.FPS));
            ...
            if(display.height() >= lineHeight * 7)
            {
                ...
                display.println(str_sprintf("CPU: %3.0f%%, %3.0f%%  ", taskManager.GetCPUUsagePercent(0),
        ...
        void Draw(Screen&display, bool bRedraw) override
        {
            ...
            if(bRedraw || lastFullDraw == 0 || millis() - lastFullDraw > 1000)
            {
                ...
                if(bRedraw || lasteffect != currentEffect || sip != currentIP)
                {
                    ...
                    display.fillRect(0, display.height() - display.BottomMargin, display.width(), display.BottomMargin,
                            backColor);
                    ...
                    String sEffect = String("Effect: ") + String(currentEffect + 1) + String("/") +
                ...
                #if ENABLE_AUDIO
                    footer = str_sprintf(" LED: %2d  Aud: %2d Ser:%2d Scr: %02d", g_Values.FPS, g_Analyzer.AudioFPS(),
            ...
            #if ENABLE_AUDIO
                {
                    ...
                    for(int iPixel = 0; iPixel < cPixels; iPixel++)
                    {
                        uint16_t color16 = iPixel > litBlocks
                                   ? BLACK16
                                   : display.to16bit(ColorFromPalette(vuPaletteGreen, iPixel * (256 / (cPixels))));
      ...
      public:
        ...
        void Draw(Screen&display, bool bRedraw) override
        {
            ...
            if(mh == 1 && mw > 32) // Only wrap if we have more than 32 LEDs in a single row
            {
                ...
                if(targetRows < 4)
                {
                    targetRows = 4; // Minimum 4 rows
                }

                if(targetRows > 16)
                {
                    targetRows = 16; // Maximum 16 rows
                }
