...
// Description:
//
//    Effect factory implementations.
//
...
std::shared_ptr<LEDStripEffect> EffectFactories::NumberedFactory::CreateEffect() const
{
    auto effect = factory();

    // Disable the effect if we have one and we were asked to do so
    if (effect && LoadDisabled)
        effect->SetEnabled(false);

    return effect;
}

EffectFactories::NumberedFactory& EffectFactories::AddEffect(EffectId effectId, const DefaultEffectFactory& defaultFactory, const JSONEffectFactory& jsonFactory, FactoryId factoryId)
{
...
void EffectFactories::ClearDefaultFactories()
{
    defaultFactories.clear();
    hashString.clear();
}

std::vector<FactoryId> EffectFactories::FactoryIDs() const
{
...
const String& EffectFactories::HashString() const
{
...
const String& EffectFactories::HashString(const String& str)
{
