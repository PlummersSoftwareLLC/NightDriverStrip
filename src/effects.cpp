...
void LoadEffectFactories()
{
    ...
    // These sections are shared by multiple projects
        ...
        RegisterAll(*g_ptrEffectFactories, Effect<StatusEffect>(CRGB::White), Effect<RainbowFillEffect>(6, 2)
            );
    #endif

    #if defined(EFFECTS_SIMPLE)
        // Simple effect set for basic LED strip projects
        RegisterAll(*g_ptrEffectFactories, Effect<FireEffect>("Medium Fire", NUM_LEDS, 1, 3, 100, 3, 4, true, true), Effect<BouncingBallEffect>(3, true, true, 1),
        ...
        RegisterAll(*g_ptrEffectFactories, Effect<PDPCMXEffect>(), Effect<PDPGridEffect>()
            );
    #endif
        ...
        RegisterAll(*g_ptrEffectFactories, Effect<FireEffect>("Medium Fire", NUM_LEDS, 1, 3, 100, 3, 4, true, true), Effect<BouncingBallEffect>(3, true, true, 1),
            Effect<BouncingBallEffect>(8, true, true, 1), Effect<MeteorEffect>(4, 4, 10, 2.0, 2.0), Effect<MeteorEffect>(2, 4, 10, 2.0, 2.0),
            ...
            Effect<PaletteEffect>(RainbowColors_p, .25, 1, 0, 1.0, 0.0, LINEARBLEND, true, 1.0), Effect<PaletteEffect>(RainbowColors_p)
            );

        #if ENABLE_AUDIO
            RegisterAll(*g_ptrEffectFactories, Effect<StarEffect<MusicStar> >("RGB Music Blend Stars", RGBColors_p, 0.2, 1, NOBLEND, 5.0, 0.1, 2.0),
        ...
        RegisterAll(*g_ptrEffectFactories, Effect<RainbowFillEffect>(24, 0), Effect<ColorCycleEffect>(BottomUp), Effect<ColorCycleEffect>(TopDown), Effect<ColorCycleEffect>(LeftRight),
            Effect<ColorCycleEffect>(RightLeft), Effect<PaletteReelEffect>("PaletteReelEffect"), Effect<MeteorEffect>(), Effect<TapeReelEffect>("TapeReelEffect"),
            Effect<StarEffect<MusicStar> >("RGB Music Blend Stars", RGBColors_p, 0.8, 1, NOBLEND, 15.0, 0.1, 10.0),
            Effect<StarEffect<MusicStar> >("Rainbow Music Stars", RainbowColors_p, 2.0, 2, LINEARBLEND, 5.0, 0.0, 10.0), Effect<FanBeatEffect>("FanBeat"),
        ...
        RegisterAll(*g_ptrEffectFactories, Effect<LanternEffect>(), Effect<PaletteEffect>(RainbowColors_p, 2.0f, 0.1, 0.0, 1.0, 0.0, LINEARBLEND, true, 1.0), Effect<RainbowFillEffect>(10, 32)
            );
    #endif

    #if defined(EFFECTS_LANTERN)
        // Lantern effect set
        RegisterAll(*g_ptrEffectFactories, Effect<FireEffect>("Calm Fire", NUM_LEDS, 40, 5, 50, 3, 3, true, true)
            );
    #endif

    #if defined(EFFECTS_FULLMATRIX)
        // Full matrix effect set for advanced displays (Mesmerizer, etc.)
        RegisterAll(*g_ptrEffectFactories, Effect<SpectrumBarEffect>("Audiograph", 16, 4, 0), Effect<SpectrumAnalyzerEffect>("Spectrum", NUM_BANDS, spectrumAltColors, false, 0, 0, 1.6, 1.6),
            Effect<SpectrumAnalyzerEffect>("AudioWave", MATRIX_WIDTH, CRGB(0, 0, 40), 0, 1.25, 1.25, true), Effect<PatternSMRadialWave>(),
            Effect<PatternAnimatedGIF>("Fire Log", GIFIdentifier::Firelog),
            Effect<PatternAnimatedGIF>("Pacman", GIFIdentifier::Pacman), Effect<PatternPongClock>(), Effect<PatternAnimatedGIF>("Colorball", GIFIdentifier::ColorSphere),
            Effect<PatternSMFire2021>(),
            Effect<GhostWave>("GhostWave", 0, 30, false, 10), Effect<PatternSMGamma>(), Effect<PatternAnimatedGIF>("Rings", GIFIdentifier::ThreeRings),
            Effect<PatternAnimatedGIF>("Atomic", GIFIdentifier::Atomic), Effect<PatternAnimatedGIF>("Bananaman", GIFIdentifier::Banana, true, CRGB::DarkBlue), Effect<PatternSMMetaBalls>(),
            Effect<PatternSMSupernova>(), Effect<PatternCube>(), Effect<PatternAnimatedGIF>("Tesseract", GIFIdentifier::Tesseract), Effect<PatternAnimatedGIF>("Nyancat", GIFIdentifier::Nyancat),
            Effect<PatternLife>(), Effect<PatternCircuit>(), Effect<SpectrumAnalyzerEffect>("USA", NUM_BANDS, USAColors_p, true, 0, 0, 0.75, 0.75),
            Effect<SpectrumAnalyzerEffect>("Spectrum 2", 32, spectrumBasicColors, false, 100, 0, 0.75, 0.75),
            Effect<SpectrumAnalyzerEffect>("Spectrum++", NUM_BANDS, spectrumBasicColors, false, 0, 40, -1.0, 2.0), Effect<WaveformEffect>("WaveIn", 8), Effect<GhostWave>("WaveOut", 0, 0, true, 0),
            Effect<StarEffect<MusicStar> >("Stars", RainbowColors_p, 1.0, 1, LINEARBLEND, 2.0, 0.5, 10.0)
            );

        #if ENABLE_WIFI
            RegisterAll(*g_ptrEffectFactories, Effect<PatternStocks>(), Effect<PatternSubscribers>(), Effect<PatternWeather>()
            );
        #endif

        RegisterAll(*g_ptrEffectFactories, Effect<PatternSMSmoke>(), Effect<GhostWave>("PlasmaWave", 0, 255, false), Effect<PatternSMNoise>("Shikon", PatternSMNoise::EffectType::Shikon_t),
            Effect<PatternSMRadialFire>(), Effect<PatternSMFlowFields>(), Effect<PatternSMBlurringColors>(), Effect<PatternSMWalkingMachine>(), Effect<PatternSMHypnosis>(),
            Effect<PatternSMStarDeep>(),
            Effect<PatternSM2DDPR>(), Effect<PatternSMPicasso3in1>("Lines", 38), Effect<PatternSMPicasso3in1>("Circles", 73), Effect<PatternSMAmberRain>(), Effect<PatternSMStrobeDiffusion>(),
            Effect<PatternSMRainbowTunnel>(), Effect<PatternSMSpiroPulse>(), Effect<PatternSMTwister>(), Effect<PatternSMHolidayLights>(), Effect<PatternRose>(), Effect<PatternPinwheel>(),
            Effect<PatternSunburst>(), Effect<PatternClock>(), Effect<PatternAlienText>(), Effect<PatternPulsar>(), Effect<PatternBounce>(), Effect<PatternWave>(), Effect<PatternSwirl>(),
            Effect<PatternSerendipity>(), Effect<PatternMandala>(), Effect<PatternMunch>(), Effect<PatternMaze>()
            );
    #endif

    #if defined(EFFECTS_UMBRELLA)
        // Umbrella-specific effects
        RegisterAll(*g_ptrEffectFactories, Effect<FireEffect>("Calm Fire", NUM_LEDS, 2, 2, 75, 3, 10, true, false), Effect<FireEffect>("Medium Fire", NUM_LEDS, 1, 5, 100, 3, 4, true, false),
            ...
            Effect<BouncingBallEffect>(), Effect<DoublePaletteEffect>(), Effect<MeteorEffect>(4, 4, 10, 2.0, 2.0), Effect<MeteorEffect>(10, 1, 20, 1.5, 1.5),
            ...
            Effect<StarEffect<Star> >("Blue Sparkle Stars", BlueColors_p, kStarEffectProbability, 1, LINEARBLEND, 2.0, 0.0, kStarEffectMusicFactor),
            Effect<StarEffect<QuietStar> >("Red Twinkle Stars", RedColors_p, 1.0, 1, LINEARBLEND, 2.0),
            Effect<StarEffect<Star> >("Lava Stars", LavaColors_p, kStarEffectProbability, 1, LINEARBLEND, 2.0, 0.0, kStarEffectMusicFactor), Effect<PaletteEffect>(RainbowColors_p),
            Effect<PaletteEffect>(RainbowColors_p, 1.0, 1.0), Effect<PaletteEffect>(RainbowColors_p, .25)
            );
    #endif

    #if defined(EFFECTS_SPECTRUM)
        // Spectrum analyzer effect set
        RegisterAll(*g_ptrEffectFactories, Effect<SpectrumAnalyzerEffect>("Spectrum Standard", NUM_BANDS, spectrumAltColors, false, 0, 0, 0.5, 1.5),
            ...
            Effect<GhostWave>("GhostWave", 0, 16, false, 15), Effect<SpectrumAnalyzerEffect>("Spectrum USA", 16, USAColors_p, true, 0), Effect<GhostWave>("GhostWave Rainbow", 8),
            Effect<SpectrumAnalyzerEffect>("Spectrum Fade", 24, RainbowColors_p, false, 50, 70, -1.0, 2.0), Effect<GhostWave>("GhostWave Blue", 0),
            Effect<SpectrumAnalyzerEffect>("Spectrum Standard", 24, RainbowColors_p, false), Effect<GhostWave>("GhostWave One", 4)
            );
    #endif

    #if defined(EFFECTS_HELMET)
        // Helmet display effect set
        RegisterAll(*g_ptrEffectFactories, Effect<SilonEffect>(), Effect<SpectrumAnalyzerEffect>("Spectrum Standard", NUM_BANDS, spectrumAltColors, false, 0, 0, 0.5, 1.5)
            );
    #endif

    #if defined(EFFECTS_TTGO)
        // TTGO display effect set
        RegisterAll(*g_ptrEffectFactories, Effect<SpectrumAnalyzerEffect>("Spectrum Fade", 12, spectrumBasicColors, false, 50, 70, -1.0, 3.0)
            );
    #endif

    #if defined(EFFECTS_WROVERKIT)
        // Wrover Kit effect set
        RegisterAll(*g_ptrEffectFactories, Effect<PaletteEffect>(rainbowPalette, 256 / 16, .2, 0)
            );
    #endif

    #if defined(EFFECTS_XMASTREES)
        // Christmas trees effect set
        RegisterAll(*g_ptrEffectFactories, Effect<ColorBeatOverRed>("ColorBeatOverRed"), Effect<ColorCycleEffect>(BottomUp, 6), Effect<ColorCycleEffect>(BottomUp, 2), Effect<RainbowFillEffect>(48, 0),
            Effect<ColorCycleEffect>(BottomUp, 3), Effect<ColorCycleEffect>(BottomUp, 1),
            ...
            Effect<StarEffect<LongLifeSparkleStar> >("Blue Sparkle Stars", GreenColors_p, 2.0, 1, LINEARBLEND, 2.0, 0.0, 0.0, CRGB::Blue), Effect<PaletteEffect>(rainbowPalette, 256 / 16, .2, 0)
            );
    #endif

    #if defined(EFFECTS_INSULATORS)
        // Insulators effect set
        RegisterAll(*g_ptrEffectFactories, Effect<InsulatorSpectrumEffect>("Spectrum Effect", RainbowColors_p), Effect<NewMoltenGlassOnVioletBkgnd>("Molten Glass", RainbowColors_p),
            Effect<StarEffect<MusicStar> >("RGB Music Blend Stars", RGBColors_p, 0.8, 1, NOBLEND, 15.0, 0.1, 10.0),
            Effect<StarEffect<MusicStar> >("Rainbow Music Stars", RainbowColors_p, 2.0, 2, LINEARBLEND, 5.0, 0.0, 10.0), Effect<PaletteReelEffect>("PaletteReelEffect"),
            Effect<ColorBeatOverRed>("ColorBeatOverRed"), Effect<TapeReelEffect>("TapeReelEffect")
            );
    #endif

    #if defined(EFFECTS_CUBE)
        // Cube display effect set
        RegisterAll(*g_ptrEffectFactories, Effect<PaletteEffect>(rainbowPalette, 256 / 16, .2, 0), Effect<SparklySpinningMusicEffect>("SparklySpinningMusical", RainbowColors_p),
            Effect<ColorBeatOverRed>("ColorBeatOnRedBkgnd"), Effect<SimpleInsulatorBeatEffect2>("SimpleInsulatorColorBeat"),
            Effect<StarEffect<MusicStar> >("Rainbow Music Stars", RainbowColors_p, 2.0, 2, LINEARBLEND, 5.0, 0.0, 10.0)
            );
    #endif
        ...
        RegisterAll(*g_ptrEffectFactories, Effect<MoltenGlassOnVioletBkgnd>("MoltenGlass", RainbowColors_p)
            );
    #endif

    #if defined(EFFECTS_ATOMLIGHT)
        // Atom light effect set
        RegisterAll(*g_ptrEffectFactories, Effect<ColorFillEffect>(CRGB::White, 1), Effect<FireFanEffect>(HeatColors_p, NUM_LEDS, 2, 2, 200, 2, 5, Sequential, true, false),
            ...
            Effect<FireFanEffect>(HeatColors_p, NUM_LEDS, 1, 10, 400, 2, NUM_LEDS / 2, Sequential, true, false, true), Effect<BouncingBallEffect>(3, true, true, 1),
            Effect<RainbowFillEffect>(60, 0),
            Effect<ColorCycleEffect>(Sequential), Effect<PaletteEffect>(RainbowColors_p, 4, 0.1, 0.0, 1.0, 0.0), Effect<MeteorEffect>(20, 1, 25, .15, .05),
            Effect<MeteorEffect>(12, 1, 25, .15, .08),
            Effect<MeteorEffect>(6, 1, 25, .15, .12), Effect<MeteorEffect>(1, 1, 5, .15, .25), Effect<MeteorEffect>()
            );
    #endif

    #if defined(EFFECTS_PLATECOVER)
        // Plate cover effect set
        RegisterAll(*g_ptrEffectFactories, Effect<ColorFillEffect>("Solid White", CRGB::White, 1), Effect<ColorFillEffect>("Solid Red", CRGB::Red, 1),
            Effect<ColorFillEffect>("Solid Amber", CRGB(255, 50, 0), 1), Effect<FireFanEffect>(HeatColors_p, NUM_LEDS, 4, 5.0, 200, 8, 8, Sequential, true, true, true, 90),
            Effect<RainbowFillEffect>(16, 3, true), Effect<MeteorEffect>(2, 1, 15, .75, .75), Effect<ColorFillEffect>("Off", CRGB::Black, 1)
            );
    #endif

    #if defined(EFFECTS_SPIRALLAMP)
        // Spiral lamp effect set
        RegisterAll(*g_ptrEffectFactories, Effect<VUMeterVerticalEffect>(), Effect<MeteorEffect>(4, 4, 10, 1.0, 1.0), Effect<ColorFillEffect>("Solid White", CRGB::White, 1),
            ...
            Effect<FireFanEffect>(HeatColors_p, NUM_LEDS, 1, 2.5, 200, 2, 15, Sequential, true, false, true), Effect<RainbowFillEffect>(120, 0),
            Effect<PaletteEffect>(RainbowColors_p, 4, 0.1, 0.0, 1.0, 0.0), Effect<BouncingBallEffect>(3, true, true, 8),
            Effect<StarEffect<MusicStar> >("Rainbow Twinkle Stars", RainbowColors_p, kStarEffectProbability, 1, LINEARBLEND, 0.0, 0.0, kStarEffectMusicFactor),
            ...
            Effect<StarEffect<MusicStar> >("Green Stars", GreenColors_p, kStarEffectProbability, 1, LINEARBLEND, 2.0, 0.0, kStarEffectMusicFactor), Effect<TwinkleEffect>(NUM_LEDS / 2, 20, 50),
            Effect<PaletteEffect>(RainbowColors_p, .25, 1, 0, 1.0, 0.0, LINEARBLEND, true, 1.0), Effect<PaletteEffect>(RainbowColors_p)
            );
    #endif
    ...
    if(g_ptrEffectFactories->IsEmpty())
        RegisterAll(*g_ptrEffectFactories, Effect<RainbowFillEffect>(6, 2)
                );
