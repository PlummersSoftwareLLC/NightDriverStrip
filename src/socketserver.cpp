    ...
    bool SocketServer::ProcessIncomingConnectionsLoop()
    {
        ...
        do
        {
            ...
            if(header == COMPRESSED_HEADER)
            {
                ...
                #if USE_PSRAM
                    std::unique_ptr<uint8_t []> _abTempBuffer = std::make_unique<uint8_t []>(MAXIMUM_PACKET_SIZE + 1); // Plus one for uzlib
                                                                                                                       // buffer overreach
                                                                                                                       // bug
            ...
            else
            {
                ...
                if(command16 == WIFI_COMMAND_PEAKDATA)
                {
                    #if ENABLE_AUDIO
                        {
                            ...
                            {
                                debugE(
                                        "Expecting %zu bytes for %d audio bands, but received %zu.  Ensure float size and endianness matches between sender and receiver systems.",
