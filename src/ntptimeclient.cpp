...
bool NTPTimeClient::UpdateClockFromWeb(WiFiUDP *pUDP)
{
    ...
    {
        debugW("Update already in progress, skipping time check, as it "
               "seems to disturb the update process.");
        return false;
    }
    ...
    // Set the ll (leap indicator), vvv (version number) and mmm (mode)
    // bits.
    //
    //  These bits are contained in the first byte of chNtpPacker and are
    //  in
    ...
    if (WiFi.hostByName(
            g_ptrSystem->DeviceConfig().GetNTPServer().c_str(),
            ipNtpServer) != 1)
        ipNtpServer.fromString(
            "216.239.35.12"); // Use Google Time as default. The
                              // pool.ntp.org servers (IPs) don't
    ...
    {
        debugE("Unexpected number of bytes back from UDP read, so "
               "returning early");
        return false;
    }

    // Obtain the time from the packet, convert to Unix time, and adjust
    // for the time zone.
    ...
    // BUGBUG (davepl): I've been getting back odd packets where the clock
    // is year 2036 and micros is 0. I ignore those.
    ...
    tvNew.tv_sec =
        ((unsigned long)chNtpPacket[40]
         << 24) +               // bits 24 through 31 of ntp time
        ((unsigned long)chNtpPacket[41]
         << 16) +               // bits 16 through 23 of ntp time
        ((unsigned long)chNtpPacket[42]
         << 8) +                // bits  8 through 15 of ntp time
        ((unsigned long)
             chNtpPacket[43]) - // bits  0 through  7 of ntp time
    ...
    double dOld =
        tvOld.tv_sec + ((double)tvOld.tv_usec / MICROS_PER_SECOND);
    double dNew =
        tvNew.tv_sec + ((double)tvNew.tv_usec / MICROS_PER_SECOND);

    debugI("Old Time: %lf, New Time: %lf, Delta: %lf", dOld, dNew,
           abs(dNew - dOld));
    ...
    {
        debugI("Clock is only off by %lf so not updating the RTC.",
               delta);
    }
    else
    {
    ...
    debugI("NTP clock: response received, updated time to: %ld.%ld, "
           "DELTA: %lf\n",
           tvNew.tv_sec, tvNew.tv_usec, dNew - dOld);
...
void NTPTimeClient::ShowUptime()
{
    ...
    case ESP_RST_POWERON:
        reason_text = "Power On";
        break;
    case ESP_RST_EXT:
        reason_text = "External Pin";
        break;
    case ESP_RST_SW:
        reason_text = "Software Restart";
        break;
    case ESP_RST_PANIC:
        reason_text = "Panic";
        break;
    case ESP_RST_INT_WDT:
        reason_text = "Watchdog barked";
        break;
    case ESP_RST_TASK_WDT:
        reason_text = "Task Watchdog barked";
        break;
    case ESP_RST_WDT:
        reason_text = "Other Watchdog barked";
        break;
    case ESP_RST_DEEPSLEEP:
        reason_text = "Reset in deep sleep";
        break;
    case ESP_RST_BROWNOUT:
        reason_text = "Brownout";
        break;
    case ESP_RST_SDIO:
        reason_text = "Reset over SDIO";
        break;
