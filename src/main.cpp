...
// terms of the General Public License (GPL). This requires that anyone
// modifying the NightDriver code (for anything other than personal use)
// or building applications based on NightDriver code must also make their
// derived product available under the same open source GPL terms. By
// purchasing a license for NightDriver, you would not then be bound by
// the GPL and you would gain an extended feature set and various levels
// of support. Think commas, not zeros, when discussing product volumes
// and license pricing.
//
...
//
// Contact Plummer's Software LLC for volume pricing and support
// questions.
//
...
// The one and only instance of ImprovSerial.  We instantiate it as the
// type needed for the serial port on this module.  That's usually
// HardwareSerial but can be other types on the S2, etc... which is why
// it's a template class.

std::unique_ptr<ImprovSerial<typeof(Serial)>> g_pImprovSerial;

// If an insulator or tree or fan has multiple rings, this table defines
// how those rings are laid out such that they add up to FAN_SIZE pixels
// total per ring.
//
// Imagine a setup of 5 Christmas trees, where each tree was made up of 4
// concentric rings of decreasing size, like 16, 12, 8, 4.  You would have
// NUM_FANS of 5 and MAX_RINGS of 4 and your ring table would be 16, 12,
// 8 4.

const int g_aRingSizeTable[MAX_RINGS] = {
    RING_SIZE_0, RING_SIZE_1, RING_SIZE_2, RING_SIZE_3, RING_SIZE_4};
...
void PrintOutputHeader()
{
    ...
    debugI("-------------------------------------------------------------"
           "-----------------------------------------------");
    debugI("M5STICKC: %d, USE_M5DISPLAY: %d, USE_TFTSPI: %d, USE_LCD: "
           "%d, AUDIO_ENABLED: %d, ENABLE_REMOTE: %d",
           M5STICKC, USE_M5DISPLAY, USE_TFTSPI, USE_LCD, ENABLE_AUDIO,
           ENABLE_REMOTE);
    ...
    debugI("Version %u: Wifi SSID: \"%s\" - ESP32 Free Memory: %u, "
           "PSRAM:%u, PSRAM Free: %u",
           FLASH_VERSION, cszSSID, ESP.getFreeHeap(), ESP.getPsramSize(),
           ESP.getFreePsram());
...
// TerminateHandler
//
// Set as the handler for unhandled exceptions, prints some debug info and
// reboots the chip!
...
void TerminateHandler()
{
    debugE("-------------------------------------------------------------"
           "------------------------");
    debugE("- NightDriverStrip Guru Meditation                           "
           "   Unhandled Exception -");
    debugE("-------------------------------------------------------------"
           "------------------------");
...
// init, then spins off worker tasks and returns control to the system so
// it can invoke the main loop() function.
//
// Threads (tasks) created here can include:
//
// DebugLoopTaskEntry           - Run a little debug console accessible
// via telnet and serial ScreenUpdateLoopEntry        - Displays stats on
// the attached OLED/TFT screen about drawing, network, etc.
// DrawLoopTaskEntry            - Handles drawing from local or wifi data
// RemoteLoop                   - Handles the remote control loop
// NetworkHandlingLoopEntry     - Connects to WiFi, handles reconnects,
// OTA updates, web server SocketServerTaskEntry        - Creates the
// socket and listens for incoming wifi color data AudioSamplerTaskEntry
// - Listens to room audio, creates spectrum analysis, beat detection,
// etc.

void setup()
{
    // Set the unhandled exception handler to be our own special exit
    // function
    ...
    // Enabling PSRAM allows us to use the extra 4MB of RAM on the
    // ESP32-WROVER chip, but it caused problems with the S3 rebooting
    // when WiFi connected, so for now, I've limited the default allocator
    // to be PSRAM only on the MESMERIZER project where it's well tested.
    ...
    // Create the SystemContainer that holds primary device management
    // objects.
    g_ptrSystem = make_unique_psram<SystemContainer>();

    // Start the Task Manager which takes over the watchdog role and
    // measures CPU usage
    auto &taskManager = g_ptrSystem->SetupTaskManager();

    esp_log_level_set("*", ESP_LOG_WARN); // set all components to an
    ...
    {
        // Looks like NVS is trash, or a future version we can't read.
        // Erase it.
    ...
    debugI("ESP-NOW initialized with MAC address: %s",
    ...
    // if we have valid compile-time creds and they differ from what was
    // persisted as compile-time creds, adopt them as the new WiFi creds
    // reality.
    if (cszSSID && strlen(cszSSID) > 0 && cszPassword)
    {
        String ct_ssid;
        String ct_password;
        if (!ReadWiFiConfig(WifiCredSource::CompileTimeCreds, ct_ssid,
                            ct_password) ||
            ct_ssid != cszSSID || ct_password != cszPassword)
        {
            debugI("Compile-time WiFi credentials differ from stored "
                   "credentials, adopting new credentials");
    ...
    // If we didn't decide to use current compile-time credentials, then
    // try to fetch Improv creds
    if (!ct_creds_selected && !ReadWiFiConfig(WifiCredSource::ImprovCreds,
                                              WiFi_ssid, WiFi_password))
    {
        debugW("Could not read Improv WiFI credentials, falling back to "
               "persisted compile-time credentials");

        // We don't have Improv creds, so read presisted compile-time
        // creds instead.
        if (!ReadWiFiConfig(WifiCredSource::CompileTimeCreds, WiFi_ssid,
                            WiFi_password))
        {
            // No persisted compile-time credentials either, so we just go
            // with what we have
            debugW(
                "Could not read persisted compile-time WiFI credentials "
                "either, falling back to what's compiled in");
            ct_creds_selected = true;
        }
    }

    // If we decided to use current compile-time credentials, then make
    // them the current config and write them to NVS
    if (ct_creds_selected)
    {
        WiFi_ssid     = cszSSID;
        WiFi_password = cszPassword;
        if (!WriteWiFiConfig(WifiCredSource::CompileTimeCreds, WiFi_ssid,
                             WiFi_password))
...
// from build flags. CONFIG_IDF_TARGET will be "esp32s3".
    ...
    g_pImprovSerial->setup(PROJECT_NAME, FLASH_VERSION_NAME, family,
                           name.c_str(), &Serial);
...
#if ENABLE_WIFI
    // We create the network reader here, so classes can register their
    // readers from this point onwards.
    //   Note that the thread that executes the readers is started further
    //   down, along with other networking threads.
    auto &networkReader = g_ptrSystem->SetupNetworkReader();

#if ENABLE_NTP
    // Register a network reader to update the device clock at regular
    // intervals
    networkReader.RegisterReader(UpdateNTPTime,
                                 (NTP_DELAY_ERROR_SECONDS) * 1000UL);
#endif
#endif

#if INCOMING_WIFI_ENABLED
    g_ptrSystem->SetupSocketServer(
        NetworkPort::IncomingWiFi,
        NUM_LEDS); // $C000 is free RAM on the C64, fwiw!
#endif
    ...
    // If we have a remote control enabled, set the direction on its input
    // pin accordingly
...
#endif
#if TTGO
        pinMode(37, OUTPUT);   // This pin layout allows for mounting a
                               // MAX4466 to the backside
        digitalWrite(37, LOW); //   of a TTGO with the OUT pin on 36, GND
                               //   on 37, and Vcc on 38
        pinMode(38, OUTPUT);
...
#include "amoled/LilyGo_AMOLED.h"
...
#if USE_TFTSPI
    // Height and width get reversed here because the display is actually
    // portrait, not landscape.  Once we set the rotation, it works as
    ...
    // Initialize the strand controllers depending on how many channels we
    // have
...
#elif HEXAGON
    // Hexagon is for a PCB wtih 271 LEDss arranged in the face of a
    // hexagon
    HexagonGFX::InitializeHardware(devices);
#elif USE_WS281X
    // LEDStripGFX is used for simple strips or for matrices woven from
    // strips
    ...
    // Due to the nature of how FastLED compiles, the LED_PINx must be
    // passed as a literal, not a variable (template stuff) Onboard PWM
    // LED

#if ONBOARD_LED_R
    ledcAttachPin(ONBOARD_LED_R,
                  1);       // assign RGB led pins to PWM channels
    ...
    TJpgDec.setCallback([](int16_t x, int16_t y, uint16_t w, uint16_t h,
                           uint16_t *bitmap) {
...
// Show splash effect on matrix
    ...
    // Start the network-dependent services.  These will be NOPs on a
    // non-wifi build.
...
//
// This is where an Arduino app spends most of its life but since we spin
// off tasks for much of our work, all that remains here is to maintain
// the WiFi connection, watch for OTA updates, and output logging
...
#if ENABLE_WIFI
            strOutput += str_sprintf("WiFi: %s, MAC: %s, IP: %s ",
                                     WLtoString(WiFi.status()),
                                     WiFi.macAddress().c_str(),
                                     WiFi.localIP().toString().c_str());
#endif

            strOutput += str_sprintf(
                "Mem: %u, LargestBlk: %u, PSRAM Free: %u/%u, ",
...
#if USE_WS281X
            strOutput +=
                str_sprintf("LED Bright: %3.0lf%%, LED Watts: %u, ",
                            g_Values.Brite, g_Values.Watts);
#endif

#if USE_HUB75
            strOutput += str_sprintf(
                "Refresh: %d Hz, Power: %d mW, Brite: %3.0lf%%, ",
                LEDMatrixGFX::matrix.getRefreshRate(),
...
#if ENABLE_AUDIO
            strOutput +=
                str_sprintf("Audio FPS: %d, MinVU: %6.1f, PeakVU: %6.1f, "
                            "VURatio: %3.1f ",
                            g_Analyzer.AudioFPS(), g_Analyzer.MinVU(),
                            g_Analyzer.PeakVU(), g_Analyzer.VURatio());
#endif

#if ENABLE_AUDIOSERIAL
            strOutput +=
                str_sprintf("Serial FPS: %d, ", g_Analyzer.SerialFPS());
#endif

#if INCOMING_WIFI_ENABLED
            auto &bufferManager = g_ptrSystem->BufferManagers()[0];
            strOutput +=
                str_sprintf("Buffer: %d/%d, ", bufferManager.Depth(),
                            bufferManager.BufferCount());
#endif

            const auto &taskManager = g_ptrSystem->TaskManager();
            strOutput += str_sprintf(
                "CPU: %03.0f%%, %03.0f%%, FreeDraw: %4.3lf",
                taskManager.GetCPUUsagePercent(0),
                taskManager.GetCPUUsagePercent(1), g_Values.FreeDrawTime);

            debugI("%s", strOutput.c_str());
        }

        // Once an update is underway, we loop tightly on
        // ArduinoOTA.handle.  Otherwise, we delay a bit to share the CPU.
