...
// Static initializers
DRAM_ATTR bool NTPTimeClient::_bClockSet =
    false;                      // Has our clock been set by SNTP?
DRAM_ATTR std::mutex
    NTPTimeClient::_clockMutex; // Clock guard mutex for SNTP client

#if ENABLE_ESPNOW

// ESPNOW Support
//
// We accept ESPNOW commands to change effects and so on.  This is a
// simple structure that we'll receive over ESPNOW.
...
// Message class
//
// Encapsulates an ESPNOW message, which is a command and an optional
// argument
class Message
{
public:
    ...
    constexpr Message() :
        cbSize(sizeof(Message)),
        command(ESPNowCommand::ESPNOW_INVALID),
        arg1(0)
    {
    }
...
// onReceiveESPNOW
//
// Callback function for ESPNOW that is called when a data packet is
// received

void onReceiveESPNOW(const uint8_t *macAddr, const uint8_t *data,
                     int dataLen)
{
    Message message;
    ...
    {
        debugE("ESPNOW Message received with wrong structure size: %d "
               "but should be %d",
    ...
    default:
        debugE("ESPNOW Message received with unknown command: %d",
...
//
// Callback function that the debug library (which exposes a little
// console over telnet and serial) calls in order to allow us to add
// custom commands.  I've added a clock reset and stats command, for
// example.
...
void processRemoteDebugCmd()
{
    ...
    else if (str.equalsIgnoreCase("stats"))
    {
        ...
        debugA("%s:%zux%d %uK", FLASH_VERSION_NAME,
               g_ptrSystem->Devices().size(), NUM_LEDS,
               ESP.getFreeHeap() / 1024);
        debugA("%sdB:%s", String(WiFi.RSSI()).substring(1).c_str(),
               WiFi.isConnected() ? WiFi.localIP().toString().c_str()
                                  : "None");
        debugA("BUFR:%02zu/%02zu [%dfps]", bufferManager.Depth(),
               bufferManager.BufferCount(), g_Values.FPS);
        debugA("DATA:%+04.2lf-%+04.2lf",
               bufferManager.AgeOfOldestBuffer(),
               bufferManager.AgeOfNewestBuffer());

#if ENABLE_AUDIO
        debugA("g_Analyzer._VU: %.2f, g_Analyzer._MinVU: %.2f, "
               "g_Analyzer.g_Analyzer._PeakVU: %.2f, "
               "g_Analyzer.gVURatio: %.2f",
               g_Analyzer.VU(), g_Analyzer.MinVU(), g_Analyzer.PeakVU(),
               g_Analyzer.VURatio());
#endif

#if INCOMING_WIFI_ENABLED
        debugA("Socket Buffer _cbReceived: %zu",
               g_ptrSystem->SocketServer()._cbReceived);
#endif
    }
...
// SetupOTA
//
// Set up the over-the-air programming info so that we can be flashed over
// WiFi
    ...
    ArduinoOTA
        ...
        .onProgress([](unsigned int progress, unsigned int total) {
            ...
            if (millis() - last_time > 1000)
            {
                ...
                pMatrix->SetCaption(str_sprintf("Update:%d%%", p),
                                    CAPTION_TIME);
#endif
            }
            else
...
// RemoteLoopEntry
//
// If enabled, this is the main thread loop for the remote control.  It is
// initialized and then called once every 20ms to pump its work queue and
// scan for new remote codes, etc.  If no remote is being used, this code
// and thread doesn't exist in the build.
...
#define WIFI_WAIT_BASE                                                   \
    4000 // Initial time to wait for WiFi to come up, in ms
#define WIFI_WAIT_INCREASE                                               \
    1000 // Increase of WiFi waiting time per cycle, in ms
...
WiFiConnectResult ConnectToWiFi(const String &ssid,
...
// ConnectToWiFi
//
// Try to connect to WiFi using either the SSID and password pointed to by
// arguments, or the credentials that were saved from an earlier call if
// no/nullptr arguments are passed.
WiFiConnectResult ConnectToWiFi(const String *ssid     = nullptr,
                                const String *password = nullptr)
{
    ...
    if (haveNewCredentials)
    {
        ...
        debugI("WiFi credentials passed for SSID \"%s\"",
               WiFi_ssid.c_str());
    }
    // If we're already connected and services are running then go no
    // further
    ...
    // (Re)connect if credentials have changed, or our last attempt was
    // long enough ago
    if (haveNewCredentials || millisAtLastAttempt == 0 ||
        millis() - millisAtLastAttempt >= retryDelay)
    {
        millisAtLastAttempt = millis();
        retryDelay          = std::min<unsigned long>(
            retryDelay + WIFI_WAIT_INCREASE, WIFI_WAIT_MAX);
        ...
        {
            auto hostname =
                g_ptrSystem->DeviceConfig().GetHostname().c_str();
            ...
            debugV("Wifi.mode");
            WiFi.mode(WIFI_STA);
            debugW("Connecting to Wifi SSID: \"%s\" - ESP32 Free Memory: "
                   "%u, PSRAM:%u, PSRAM Free: %u\n",
                   WiFi_ssid.c_str(), ESP.getFreeHeap(),
    ...
    if (WiFi.isConnected())
    {
        debugW("Connected to AP with BSSID: \"%s\", received IP: %s",
               WiFi.BSSIDstr().c_str(),
               WiFi.localIP().toString().c_str());
    }
    else
    // Additional services onwards are reliant on network so return if
    // WiFi is not up (yet)
    {
    ...
    // If we were connected before, network-dependent services will have
...
void UpdateNTPTime()
{
    ...
    if (WiFi.isConnected())
    {
        // If we've already retrieved the time successfully, we'll only
        // actually update every NTP_DELAY_SECONDS seconds
        if (!NTPTimeClient::HasClockBeenSet() ||
            (millis() - lastUpdate) > ((NTP_DELAY_SECONDS) * 1000))
        {
...
// ProcessIncomingData
//
// Code that actually handles whatever comes in on the socket.  Must be
// known good data as this code does not validate!  This is where the
// commands and pixel data are received from the server.

bool ProcessIncomingData(std::unique_ptr<uint8_t[]> &payloadData,
                         size_t payloadLength)
{
    ...
    case WIFI_COMMAND_PEAKDATA: {
        ...
        debugV("ProcessIncomingData -- Bands: %u, Length: %u, Seconds: "
               "%llu, Micros: %llu ... ",
               numbands, length32, seconds, micros);

        // Data is transmitted as NUM_BANDS floats following the standard
        // header
        const uint8_t *dataStart =
            payloadData.get() + STANDARD_DATA_HEADER_SIZE;
        const size_t availableFloats =
            (payloadLength > STANDARD_DATA_HEADER_SIZE)
                ? (payloadLength - STANDARD_DATA_HEADER_SIZE) /
                      sizeof(float)
                : 0;
        const size_t copyCount = std::min<size_t>(
            NUM_BANDS, std::min<size_t>(numbands, availableFloats));

        PeakData peaks{}; // zero-initialized for any bands not provided
        if (copyCount > 0)
        {
            const float *pFloats =
                reinterpret_cast<const float *>(dataStart);
    ...
    case WIFI_COMMAND_PIXELDATA64: {
        ...
        debugV("ProcessIncomingData -- Channel: %u, Length: %u, Seconds: "
               "%llu, Micros: %llu ... ",
               channel16, length32, seconds, micros);

        // The very old original implementation used channel numbers, not
        // a mask, and only channel 0 was supported at that time, so if we
        // see a Channel 0 asked for, it must be very old, and we massage
        // it into the mask for Channel0 instead Another option here would
        // be to draw on all channels (0xff) instead of just one (0x01) if
        // 0 is specified

        if (channel16 == 0)
            channel16 = 1;

        // Go through the channel mask to see which bits are set in the
        // channel16 specifier, and send the data to each and every
        // channel that matches the mask.  So if the send channel 7, that
        // means the lowest 3 channels will be set.

        std::lock_guard<std::mutex> guard(g_buffer_mutex);

        for (int iChannel = 0, channelMask = 1;
             iChannel < g_ptrSystem->BufferManagers().size();
             iChannel++, channelMask <<= 1)
        {
            if ((channelMask & channel16) != 0)
            {
                ...
                bool bDone = false;
                auto &bufferManager =
                    g_ptrSystem->BufferManagers()[iChannel];

                if (!bufferManager.IsEmpty())
                {
                    auto pNewestBuffer = bufferManager.PeekNewestBuffer();
                    if (micros != 0 &&
                        pNewestBuffer->MicroSeconds() == micros &&
                        pNewestBuffer->Seconds() == seconds)
                    {
...
// ReadWiFiConfig
//
// Attempts to read the WiFi ssid and password from NVS storage strings.
// The keys for those name-value pairs are made from the variable names
// (WiFi_ssid, WiFi_Password) directly.  Limited to 63 characters in both
// cases, which is the WPA2 ssid limit.

bool ReadWiFiConfig(WifiCredSource source, String &WiFi_ssid,
                    String &WiFi_password)
{
    ...
    {
        // Read the SSID and Password from the NVS partition name/value
        // keypair set

        auto len = std::size(szBuffer);
        err      = nvs_get_str(
            nvsROHandle,
            GetWiFiConfigKey(source, NAME_OF(WiFi_ssid)).c_str(),
            szBuffer, &len);
        if (ESP_OK != err)
        {
            debugE("Could not read WiFi_ssid for source %d from NVS",
                   source);
        ...
        err = nvs_get_str(
            nvsROHandle,
            GetWiFiConfigKey(source, NAME_OF(WiFi_password)).c_str(),
            szBuffer, &len);
        if (ESP_OK != err)
        {
            debugE("Could not read WiFi_password for SSID \"%s\" and "
                   "source %d from NVS",
                   WiFi_ssid.c_str(), source);
            nvs_close(nvsROHandle);
            return false;
        }
        WiFi_password = szBuffer;

        // Don't check in changes that would display the password in logs,
        // etc.
        debugI("Retrieved SSID and Password for source %d from NVS: "
               "\"%s\", \"********\"",
               source, WiFi_ssid.c_str());
...
// WriteWiFiConfig
//
// Attempts to write the WiFi ssid and password to NVS storage strings.
// The keys for those name-value pairs are made from the variable names
// (WiFi_ssid, WiFi_Password) directly.  It's not transactional, so it
// could conceivably succeed at writing the ssid and not the password (but
// will still report failure).  Does not enforce length limits on values
// given, so conceivable you could write longer pairs than you could read,
// but they wouldn't work on WiFi anyway.

bool WriteWiFiConfig(WifiCredSource source, const String &WiFi_ssid,
                     const String &WiFi_password)
{
    nvs_handle_t nvsRWHandle;

    // The "storage" string must match NVS partition name in partition
    // table
    ...
    err = nvs_set_str(
        nvsRWHandle, GetWiFiConfigKey(source, NAME_OF(WiFi_ssid)).c_str(),
        WiFi_ssid.c_str());
    if (ESP_OK != err)
    {
    ...
    if (success)
        ...
        debugI(
            "Stored SSID and Password for source %d to NVS: %s, *******",
            source, WiFi_ssid.c_str());
...
// ClearWiFiConfig
//
// Attempts to erase the WiFi ssid and password for a given source from
// NVS storage. The keys for the name-value pairs are constructed based on
// the source. This operation is not transactional; it's possible for one
// key to be erased successfully while the other fails.
...
void IRAM_ATTR DebugLoopTaskEntry(void *)
{
    ...
    Debug.setCallBackProjectCmds(
        &processRemoteDebugCmd); // Func called to handle any debug
                                 // extensions we add
    ...
    Debug.begin(WiFi.getHostname(),
                RemoteDebug::INFO); // Initialize the WiFi debug server
...
//
// Repeatedly calls the code to open up a socket and receive new
// connections
    ...
    for (;;)
    {
            ...
            leds != nullptr)
        {
            if (socket >= 0)
            {
                ...
                // Potentially too large for the stack, so we allocate it
                // on the heap instead
                std::unique_ptr<ColorDataPacket> pPacket =
                    std::make_unique<ColorDataPacket>();
                ...
                if (!_viewer.SendPacket(socket, pPacket.get(),
                                        sizeof(ColorDataPacket)))
                {
                    // If anything goes wrong, we close the socket so it
                    // can accept new incoming attempts
...
void IRAM_ATTR NetworkHandlingLoopEntry(void *)
{
    ...
    for (;;)
    {
        // Wait until we're woken up by a reader being flagged, or until
        // we've reached the hold point
        ulTaskNotifyTake(pdTRUE, notifyWait);

        // Every second we check WiFi, and reconnect if we've lost the
        // connection. If we are unable to restart it for any reason, we
        // reboot the chip in cases where its required, which we assume
        // from WAIT_FOR_WIFI.
        ...
        EVERY_N_SECONDS(1)
        {
            ...
            if (connectResult == WiFiConnectResult::Connected)
            {
                ...
                // It's recommended to clean up any stale web socket
                // clients every second or so
...
#if WAIT_FOR_WIFI
                // Reboot if we've been waiting for a connection for more
                // than the maximum delay between connection retries and
                // we _do_ have credentials
                if (connectResult != WiFiConnectResult::NoCredentials &&
                    millis() - millisAtLastConnected > WIFI_WAIT_MAX)
                {
                    debugE("Rebooting in 5 seconds due to no Wifi "
                           "available.");
                    delay(5000);
                    throw new std::runtime_error(
                        "Rebooting due to no Wifi available.");
                }
#endif
            }
        }

        // If the reader container isn't available yet or WiFi isn't up
        // yet, we'll sleep for a second before we check again
        ...
        for (auto &entry : networkReader.readers)
        {
            ...
            // The last check captures cases where millis() returns bogus
            // data; if the delta between now and lastReadMs is greater
            //   than the interval then something's up with our
            //   timekeeping, so we trigger the reader just to be sure
            if (interval &&
                (targetMs <= now || (std::max(now, targetMs) -
                                     std::min(now, targetMs)) > interval))
                entry.flag.store(true);

            // Unset flag before we do the actual read. This makes that we
            // don't miss another flag raise if it happens while reading
        ...
        // Calculate how long we can sleep. This is determined by the
        // reader that is closest to its interval passing.
        for (auto &entry : networkReader.readers)
        {
            if (entry.canceled.load())
                continue;
            ...
            // If one of the reader intervals passed then we're up for
            // another read cycle right away, so we can stop looking
            // further
            ...
            {
                unsigned long entryHoldMs =
                    std::min(interval, interval - (now - lastReadMs));
...
size_t NetworkReader::RegisterReader(const std::function<void()> &reader,
                                     unsigned long interval, bool flag)
{
